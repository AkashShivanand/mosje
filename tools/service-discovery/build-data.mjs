...
const PERSONAS = ${js(m.personas.map(({ id, label, short, sub, portal }) => ({ id, label, short: short || label, sub, ...(portal ? { portal: { route: portal.route, blurb: portal.blurb } } : {}) })))};
...
/* Group artwork. The live persona panel draws each group as an illustrated
   ...
   carries the group. */
const HAS_ART = new Set(${js(m.personas.filter((p) => p.art !== false && ["sc","obc","dnt","safai","senior","tg","student","ngo"].includes(p.id)).map((p) => p.id))});
...
export const SD_PERSONAS: SdPersona[] = ${js(m.personas.map(({ id, label, short, sub, portal }) => ({ id, label, short: short || label, sub, ...(portal ? { portal: { route: portal.route, blurb: portal.blurb } } : {}) })))};
