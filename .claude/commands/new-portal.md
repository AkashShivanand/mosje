...
7. **Figma handoff page:** create the portal's page on the handoff structure from day one — zones `A · START HERE` / `B · JOURNEYS` / `C · BUILD REFERENCE` / `D · ARCHIVE`, role lanes, tens-numbered flows (`.claude/rules/figma-handoff-page-structure.md`, procedure in the `figma-page-organiser` skill) — and register it in `tools/figma-handoff-structure/pages.json` so `npm run check:figma-handoff` measures it.
