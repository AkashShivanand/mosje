...
export const ANCHORS = {
  ...
  "primaryRamp.navy": {
    anchor: "#003366",
    anchorStep: 600,
    // 96.8, not 98.5 (2026-09-14): at 98.5 rung 50 was #f7faff, CIE L* 98.2 — LIGHTER than the
    // page canvas and 1.05:1 on white, so every `bg/brand/primary/base` fill (the sidebar's
    // route to the current page among them) was invisible under Navy. 96.8 lands rung 50 at
    // L* 96.3, beside Blue's 95.9 and the functional ramps' 95.8-96.4. Rungs 600 and below
    // keep their values; 100-500 move by at most 1.8 L*.
    lightest: 96.8,
    darkest: 12,
    note:
  ...
  dangerRamp: {
    ...
    anchor: "#ec4e4f",
    anchorStep: 400,
    // 94 STAYS, and rung 50 (#ffe4e1, L* 92.7) is deliberately the heaviest tint in the system.
    // Lifting it to the L* 96 the other ramps share was measured on 2026-09-14 and rejected: the
    // chroma arc thins as lightness rises, so at lightest 97 the error tint lands 0.4 dE from India
    // Saffron's rung 50 (3.4 at 94) and rung 100 falls from 4.5 to 2.2 dE — an error banner that
    // reads as a saffron brand wash. An error also carries more urgency than a note, so a heavier
    // ground is the right weight, not an inconsistency.
    lightest: 94,
    darkest: 19.5,
    note:
 ...
 * binds, so `color/transparent/<family>/0` is declared in ZERO_TIERS below together with its
 * consumer. None is declared today: secondary had one from 2026-09-06 for the rail's identity
 * wash, and it went with the wash on 2026-09-15. In Figma a bound gradient stop takes its
 * variable's alpha, which is why "transparent" has to be a variable.
 */
...
export function buildAlphaTiers() {
  ...
  // family -> description. Empty: no fade on the estate needs a bound far end.
  const ZERO_TIERS = {};
  const BASE = {
  ...
  const out = {
    $description:
      `Alpha overlay tiers (${ALPHA_STEPS.join("/")}%). A 0 tier — the far end of a fade — is added ` +
      `per family only WITH its consumer (check:token-consumers refuses a token nothing binds); none ` +
      `exists today. GENERATED by build/brand-ramps.mjs. ` +
  ...
  for (const [family, base] of Object.entries(BASE)) {
    const tier = {};
    // A 0 tier exists only where a consumer needs a fade's far end. It is emitted HERE, not
    // hand-added to semantic.json: until 2026-09-14 it was hand-added, so every run of this
    // generator silently deleted the stop the sidebar's identity wash binds.
    if (ZERO_TIERS[family]) {
      tier["0"] = { $value: base, $extensions: { mosje: { alpha: "{alpha.0}" } }, $description: ZERO_TIERS[family] };
    }
