...
export function buildAlphaTiers() {
  ...
  const ZERO_TIERS = {
    secondary: "The far end of a fade: secondary at 0%. A gradient that dissolves into nothing needs a bound stop of the SAME hue at zero alpha — in Figma a bound gradient stop takes its variable's alpha, so `transparent` has to be a variable too. Never used as a fill on its own.",
  };
  const BASE = {
  ...
  const out = {
    $description:
      `Alpha overlay tiers (${ALPHA_STEPS.join("/")}%), plus a 0 tier where a fade needs its far end ` +
      `(secondary, for the rail's identity wash, 2026-09-06). A 0 tier is added per family WITH its ` +
      `consumer — check:token-consumers refuses a token nothing binds. GENERATED by build/brand-ramps.mjs. ` +
