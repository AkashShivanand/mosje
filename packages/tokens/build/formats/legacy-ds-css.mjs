...
export const legacyDsCss = {
  ...
  format: ({ dictionary }) => {
    ...
    /**
     * REDUCED MOTION, ONCE, AT THE TOKEN LAYER.
     *
     * Every `motion/<intent>/duration` collapses to 0.01ms when the reader has asked the
     * operating system for less motion — so a component that binds a pair honours WCAG 2.3.3
     * without writing its own media query (45 components had, each slightly differently).
     * 0.01ms rather than 0 so `transitionend` still fires for code that waits on it.
     *
     * Two families are deliberately left out: `instant` is already zero, and `loading` —
     * a spinner that stops is a page that looks frozen, so components SLOW those instead
     * (button.css runs its spinner at 1.6s under the same query). `stagger/step` collapses
     * with the rest: a cascade with no motion is a list that simply appears.
     */
    const reducedMotionLines = dictionary.allTokens
      .filter((t) => t.path[0] === "motion" && tierOfFile(t.filePath) === "sys")
      .filter((t) => t.path[1] !== "loading" && t.path[1] !== "instant")
      .filter((t) => t.path.at(-1) === "duration" || t.path[1] === "stagger" && t.path[2] === "step")
      .map((t) => `    ${cssNameFor(t)}: 0.01ms;`);
    const reducedMotionBlock = reducedMotionLines.length
      ? `/* ---- prefers-reduced-motion: every motion intent collapses here; loading and instant are exempt (see legacy-ds-css.mjs) ---- */\n` +
