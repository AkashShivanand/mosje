...
const MAP = {};
const put = (path, ref, description, extensions) => {
  MAP[path.join("/")] = { path, ref, description, extensions };
};

/**
 * Navy-only overrides, keyed by path. Navy's key colour #003366 sits at rung 600 but is as dark
 * as Blue's rung 800 (OKLab L 32 against 35), so every role bound to rung 800 lands about two
 * shades deeper under Navy than under Blue — at the floor of what sRGB can show as a hue. Two
 * roles read as black because of it, measured 2026-09-15 in the running estate: the website
 * footer ground (`#001735`) and the visited link, 6.9 dE from body text. Under Navy they take
 * the key colour itself. The pressed button binds rung 800 directly and is deliberately NOT
 * moved: a momentary state, and lifting it would erase its step from the resting button.
 * Blue and the DBIM previews are unchanged.
 */
const NAVY = {
  "bg/brand/primary/boldest": "{color.primaryRamp.navy.600}",
  "text/link/visited/default": "{color.primaryRamp.navy.600}",
};
...
for (const { path, ref, description } of Object.values(MAP)) {
  ...
  if (navy) node[path.at(-1)].$extensions = { mosje: { colorModes: { navy } } };
}
