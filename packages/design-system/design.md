...
**Key props**: `items` (`{ id?, title, description?, href, linkLabel? }[]`) · `label` (default `"Latest Updates"`) · `icon` (slot, defaults to Material Symbols `campaign`) · `action` (slot) · `orientation` (`horizontal` default | `vertical`) · `interval` (default 5000) · `autoplay` · `linkAs`
