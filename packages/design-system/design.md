...
**Motion encodes state; it does not decorate — and NOTHING may start or stop abruptly.** The seal is **still at rest** and turns at **one speed, 6s**, whenever the widget is engaged: hovered, focused, or thinking. It reports work in progress even when the panel has scrolled the typing dots out of view.
...
- **Never add or remove a transform animation to express a state.** Removing the float snapped the mascot back to centre from wherever in its cycle it was, up to the full 3px. The float now **always runs and its AMPLITUDE transitions** — `--ds-chatbot-lift`, a registered `@property` with a `<length>` syntax so it can be interpolated, 0px at rest and 3px engaged. Continuous both ways, and no moment where a transform switches on or off.
