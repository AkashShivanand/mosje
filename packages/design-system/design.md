...
- **DBIM 3.0: 79 / 100** against every Annexure F Checklist 1 item that applies to a footer (29 items: 20 met, 6 partial, 3 not met), with all six §5.6 mandatory elements and the prescribed lineage present on both variants. Not met: icon colour (a light tint, not inclusive white), icons from the DBIM Toolkit (Material Symbols is an estate decision), and an Archives link (no Archives page). Partial: the pin icon's font format and 16px size, 16px Semi Bold column headings (not a DBIM Table 3 row), tinted parent-organisation lines, Help linking to Contact Us rather than help resources, and the 196 KB National Emblem file. The item-by-item table and the coverage of the live dosje.gov.in footer are on the component's documentation page.
