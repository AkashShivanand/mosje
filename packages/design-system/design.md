<!--
  ...
  `check:org-logos` is a per-file ratchet over the 83 literals that remain, and
...
- **A control on the label row is `labelAction`, NEVER a link inside `label`.** "Forgot Password?" beside the Password label is the case it exists for. Consumers used to pass a `<span>` holding the label text and a floated `<a>` as `label` — which puts an interactive element inside a `<label>`, so a click near it moves focus to the field instead of following the link, and the float never reached the right edge anyway: `.ds-field__label-row` is a flex row, so the `<label>` inside it shrink-wraps to its own text (186px in a 340px field). That shipped on the portal login pages as **"Password \*Forgot Password?"**, jammed together, and was found by opening the page rather than by any gate. `labelAction` renders as a SIBLING of the label, pushed right with `margin-left: auto` — so a `labelHelp` toggle stays beside the label where it belongs, and no field without an action moves at all.
- **Keep a `labelAction` short, and make it a real link or button.** It is not a place for instructions; those are `hint` or `labelHelp`.
...
**FIGMA PARITY: `Action Banner` on its own page in the Feedback & Status group — Variant (Banner|Card) x Breakpoint (Desktop|Mobile), four variants, zero unbound values.** Built 2026-09-07; before that the component was code-only and its docs page read "Not yet published in the Figma library." Three things are recorded on the page's Component record rather than papered over. **The Card has no breakpoint of its own** — only the banner has a media query (640), so the two Card variants draw the same layout at two cell widths; the axis is there to complete the matrix, and collapsing it would make the set harder to read, not simpler. **Two CSS values Figma cannot hold**: the banner sentence's `max-width: 60ch` (no `ch` unit) and the card's `height: 100%` (no percentage height) — the master hugs, and the arrangements section demonstrates the stretch-and-pin with three cards of different description lengths in a fixed-height row. **There is still no Code Connect template**, so Dev Mode resolves the master but hands over no snippet.
**Rules**:
...
**Contrast — the ground is `primaryScale/600`, not `/500`, and that is a fix rather than a preference.** White on `/500` (`#0373df`) measures **4.64:1** — it clears WCAG 1.4.3's 4.5:1 by four hundredths — and the moment secondary text is dimmed it fails outright: 90% is **4.06:1**, 80% is **3.52:1**. This component shipped an 80% description line on that ground and its own documentation claimed 6.3:1, which was simply wrong. On `/600` (`#005eb9`) the primary line is **6.36:1** and 80% is **4.66:1**, so a title-over-subtitle structure is possible at all. **Since 2026-09-14 the subtitle is not dimmed at all:** the library binds kind, separator and date to the same inverse text variable as the title, and the two lines are told apart by style — `Body/body-3` under `Body/body-2-semibold` — at the full 6.36:1. The plinth moves to `/800` with it, holding the tonal step at 1.77:1 against the 1.85:1 the two had before — the shape is unchanged.
...
**The panel is built from the bar's parts, deliberately**: its header is the bar's **navy plinth** — the same `primaryScale/800` behind the same mark and name — over the same `primaryScale/600` ground in the same single ink, and its rows are set in the bar's own notice style (`Body/body-2-semibold` — body-2 at 600; until 2026-09-14 this said `title-3` at 500, a combination no text style draws). Side by side the two read as one component in two shapes, which is what they are. It carried its own treatment (a transparent header over a hairline rule) until 2026-08-25 and looked like a different component wearing the same colours. **There is no rule under the header** — the plinth's colour change already separates it, and a hairline on top is a second boundary doing the first one's job.
...
- **Corners, unless it bleeds.** A contained bar carries `--sa-shape-12` (the radius the vertical panel already uses) and clips to it; a `bleed` bar keeps square ends, because a band meant to be continuous with the page edges has no corners a reader can see and rounding them would show two slivers of page through the ends. `overflow: clip`, not `hidden` — the second makes a scroll container a keyboard user can end up scrolling.
...
**Figma**: SAMAVESH › Ticker — `Breakpoint` (Desktop · Tablet · Mobile) × `Direction` (Horizontal · Vertical) × `Motion` (Playing A · Playing B · Paused A · Paused B). The A/B pair is what makes the file a working prototype rather than a still: `Playing A` advances to `Playing B` on a 5s delay and back again, both with Smart Animate, so the strip cycles on its own in presentation mode and the pause control actually stops it. `Paused A` / `Paused B` exist so a citizen can pause **and then still step** — a single Paused variant would have left prev and next with nowhere to go. Interaction states live in the nested parts — `Ticker / Row` and the library's `IconButton` and `Button` — rather than as axes on the parent, which would have multiplied 24 variants into hundreds. **Two motion mechanisms, each where it fits:** the bar uses prototype reactions (Smart Animate on the component's own 240ms ease-out, with hover states at 150ms — the same two numbers the CSS uses) so the file cycles by itself in presentation mode; the panel uses **Figma Motion** — a `TRANSLATION_Y` keyframe track on its Track, linear, because a timed variant swap cannot express a continuous scroll and any easing would make the marquee hesitate at the loop point.
...
**Closed (2026-09-07)**: the Figma `Button` set DOES carry an inverse axis — `Tone: Default | Inverse`, alongside Size / Type / Sub-type / State. The 2026-08-25 note claiming otherwise was read off the MoSJE Handoff file's Button, which has no `Tone`, rather than SAMAVESH's own. `Ticker / Action` remains a local part for now, but it no longer has to be: a `Tone=Inverse` Button instance is the correct replacement whenever Ticker is next touched.
**On the website**: the panel is the **right-hand rail of Our Offerings**, not a strip under the hero. It ran full-bleed under the hero until 2026-08-25, which stacked a third full-width coloured band beneath the saffron SAMAVESH bar and the hero, and put a second pause control 65px below the carousel's own — two controls on one screen that both stop something, neither saying what. The section's card grid reflows 3-up to 2-up at **exactly the same 408px card width** (a 12-column grid on `gap-6` in the 1272 container gives 84px columns: 8 columns = 840 = two 408 cards plus one gutter), so the rail costs the cards nothing.
...
- **The figure and its caption are one block, with no gap of their own.** Their line boxes already carry the air: `headline-2` is 40px for 32px of type and `body-2` is 20 for 14, so four points sit under the figure's baseline and three above the caption's cap before anything is added. A grid gap on top of that reads as blank space between two texts rather than as a pair — the same note `compact`'s `gap: 0` has carried since the component was built.
- **The full column count waits until 1280, and that is measured.** An extended cell must hold a 24px mark, a 12px gap and the widest figure beside them. `345,703,321` is 183px at `headline-2`; four-up inside a 1024 content column gives a 212px cell, so the copy column would be 176 — six pixels short. Two-up from 768, one below. **A published figure may never be shrunk, wrapped or clipped to fit a layout**; if it does not fit, the layout changes.
...
- **`lineage`, `policyLinks`, `sitemap`, `help` and `copyright` are REQUIRED PROPS on both variants.** A footer without them is not a government footer, and making them optional would let a caller ship one that is not.
- **There is no support strip.** A call to action is page content and the footer is statutory chrome, so the website renders it with `ActionBanner` on a light band above the footer. Neither the component nor the Figma master draws one.
- **THE FOOTER OWNS NO WIDTH — and `maxWidth` has no default.** On `website` each band's content carries `.sa-container`, so the cap (1200 / 1320 / 1440), the margin (16 / 24 / 32) and the right-wall gutter come from `foundations/layout.css`; on `portal` the bands are fluid and pad with `--sa-grid-margin-page`, as a portal `SiteHeader` does. It used to default to a restated 1280px cap with a 24px floor, which put its left edge **20px right of the masthead's at 1440 and 8px right at 375**, and disagreed with the Figma master. Do not pass `maxWidth`.
- **List rows are flex boxes.** A block `<li>` wrapped the inline-flex link in a line box sized by the inherited body line-height, so the link columns ran on a 32px rhythm instead of 28 (a 20px line + the 8px gap) — the value the tokens and the master both describe. Measured and fixed 2026-09-15.
- **It is structural, not content-bound.** Every label, href, logo and sentence arrives as a prop. Never fork it to change wording.
- **NEVER pass a background through `className`.** The ground is `bg/brand/primary/boldest` and the lead ink `on/bg/brand/primary/boldest`; every other colour is a `cmp/sitefooter/*` token — `ink/subtle` (links, 100), `ink/subtler` (boilerplate, 200), `rule/base` and `rule/subtle` (rung 500 at alpha 64 / 32 over the ground), `chip/default` and `chip/hover` (rung 100 at alpha 16 / 24) — plus `chip/size` (40) and `mark/height` (28). Code and the Figma master bind the same tokens, so the footer repaints for `blue`, `navy`, `dbim` and the five DBIM hues with no work at the call site. In `dbim` the ground resolves to **#162F6A**, DBIM's own published Blue shade 1, satisfying **[DBIM 5.6]** by construction.
- **ONE ground, ONE hairline.** The two bands ran rungs 800 and 900 for a while; a 1.28:1 step is not a distinction, it is a smudge.
- **Ink is same-hue, never white-alpha.** Three levels: `on/bg/brand/primary/boldest`, `cmp/sitefooter/ink/subtle` and `ink/subtler`. Worst case across all ten brand modes is 5.37:1 (dbim-green, dim ink) against the 4.5:1 AA asks for; the social glyph's worst case on its chip is 4.88:1 (dbim-green) against 1.4.11's 3:1. Measured in the browser 2026-09-16.
...
- **Do not put the same destination behind two labels.** The link graph is deduplicated deliberately: "Vision & Mission" and "Help & Support" both pointed at pages already linked one line above them. The two remaining shared destinations are role-distinct and intended (the `ActionBanner` CTA above the footer vs the Help nav entry; the Digital India related-link vs its mandated credit logo).
...
- **FIGMA PARITY: `Site Footer` on the library's `Footer` page — Variant (Website|Portal) x Breakpoint (Desktop|Tablet|Mobile), six variants, Code Connect template `site-footer.figma.ts`.** Verified 2026-09-16 by rendering all six from code and Figma at the same width with the same content: **heights identical (490, 794, 1138, 181, 273, 397)**, every text line, logo and chip within 4px — the residue is horizontal glyph-advance drift along a row, which no browser/Figma pair eliminates. **No master layer binds a primitive**: the Palette rungs the inks, rules and chips used (including overrides inside the glyph instances) now bind the semantic and `cmp/sitefooter/*` tokens, and zero fills, strokes, paddings, gaps, radii or text styles are unbound. Content widths bind `container/page` and `grid/margin/page` per Viewport mode; hairlines are included in layout, as a CSS border is. What is still drawn rather than bound, and recorded on the Component record: the 96ch lineage measure (a 659px max width), the 375 address break that CSS `text-wrap: balance` produces (a 180px width on the Mobile variant), and the credit boxes' widths (77 and 71.7, the images' own aspect at 28px). **THE CREDIT MARKS ARE VECTORS NOW, ON BOTH SIDES.** `NeGD/on-dark` in the library is the mark vectorised from the high-resolution artwork, its counters and the gap around the 'e' cut as TRUE HOLES so it carries no background with it, with the tagline set in Carlito — the metric twin of the logo's own Calibri — and outlined. It is exported to `NeGD-Logo-White.svg` (30 KB) and that is what the footer credit draws, replacing a 258 KB SVG that wrapped two rasters. The same geometry in NeGD's blue and green is `NeGD-Logo-Colour.svg`, which fixed a live defect: the old file painted an all-white layer over the colour one, so on the website's light `LogoStrip` the mark rendered white on white and was invisible. **The Digital India credit is the canonical mark too.** `Digital-India-White.svg` was a crop with the swirl squashed into an 86x48 box (aspect 1.79 against the artwork's 2.56) and the blue stem of the 'i' clipped by the viewBox; the credit now draws `Digital-India-Reverse.svg`, which is the estate's own `digital-india-logo.svg` with the wordmark reversed to white and the swirl left in its tricolour, because the swirl is the mark's identity and not ours to recolour. **The Figma master was worse than the code and two defects were found rebuilding it**, both in the Website/Desktop variant and both from a pass that had skipped it: its Digital India mark had no `mark ·` wrapper at all and drew a 36x16 instance, and where the other five clipped a 56.3x56.3 SQUARE-stretched instance into a 50x28 box, and its two credit prefixes were a hard `#ffffff` where the other ten bind `cmp/sitefooter/ink/subtler`. All six marks are rebuilt ink-to-ink against the SVG's own geometry — box 71.71x28, ink 71.19x27.32 at (0.45, 0.56) — and the two prefixes are bound. Still open: **VisitorCounter has no master**, so the footer draws what it renders (Body 3 label + Label 2 value); **Footer - Bottom Strip** survives for ten `PortalLoginTemplate` instances; the original `NeGD-Logo.svg` (258 KB) and `Digital-India-White.svg` are RETIRED — deleted 2026-09-16 along with their two lines in `download-assets.mjs`, because the scrape manifest writes to the same folder and would have re-downloaded both on its next run. **RE-MEASURED 2026-09-16 AGAINST THE RUNNING SITE, and every remaining difference is one of two runtime affordances the master deliberately does not draw — both website-only, which is why the three PORTAL variants match to the pixel (181 / 273 / 397).** (1) The **scroll-end safe area**: below 1336px the footer reserves `--sa-corner-safe-bottom` (116px) under the statutory band so the fixed corner launcher cannot park on the copyright line, which is **+92px** at 768 and at 375. (2) The **right-wall gutter**: the website's `.sa-container` pads its inline end with `--sa-wall-clearance` (48px), so at 375 the code has 311px of content where the master draws 343 — the master therefore keeps "Ministry of Social Justice & Empowerment" on one line and fits the six policy links in two rows, where the browser takes two lines and three rows, **+44px**. So Website Tablet is 886 against 794 and Website Mobile 1274 against 1138, and both reconcile exactly: 886 − 92 = 794, and 1274 − 92 − 44 = 1138. Drawing either in Figma would mean typing an unbound 48 and 116 into a master whose paddings all bind `grid/margin/page`, so they stay undrawn and stay written down here.
...
- **The GROUND belongs to the CONTAINER, not the mark — changed 2026-09-06.** `OrgLogo` carried a `tile` boolean (white ground, hairline rule, `shape/8`) that was ON by default, and **seventeen of its twenty-six call sites had to switch it off**. A property most callers must remember to turn off is the wrong default and the wrong home for the decision, and in Figma it put a toggle on every instance of an 18-variant set.
- **Where a mark needs a ground, put `.ds-org-tile` on the element that contains it.** ONE class, in `org-logo.css`, and it is one class for a reason: four surfaces drew this tile by hand at three different radii and two different sizes before any of it existed. Compose it — never re-derive a ground in a consumer's own stylesheet. `PortalCard` and `PortalLoginShell` are the two product surfaces that use it; the sidebar rail deliberately does not, because its SAMAVESH wash is the mark's ground there.
- **Sizes are still the mark's** — 32 / 48 / 56, and the inset survives the tile's removal so a mark never touches the edge of its box.
...
**Rules**:
- **Selected is not a state, which is why it is its own axis.** It is persistent — *this is the portal you are in* — while hover, pressed and disabled are momentary or conditional. Selected draws a **2px rule and a check, and does not tint the card**: a saffron→white wash sat under it until 2026-09-06 and was removed to match the Figma master, because the rule and the check already say "this one" and a third signal for one fact only put ink on a moving ground.
- **Focus is a boolean, not a state value.** Focus can land on a card that is also hovered, pressed, selected or disabled, so it cannot share an axis with them — same shape as `Tabs / Tab`. The ring takes the DARKER `--sa-color-secondaryScale-600`, because a focus indicator owes 3:1 against both the card it surrounds and the page behind it.
- **`disabled` keeps the card's border and its name.** The portal exists; this reader cannot open it. It renders `aria-disabled` with the `href` dropped — never the native attribute (which leaves the tab order) and never omission (which tells a screen-reader user nothing). **A disabled card owes a reason in the surrounding copy**; the component cannot know it, and a greyed card with no explanation reads as a bug.
- **Hover is gated to `(hover: hover) and (pointer: fine)`; the press is not.** Ungated, `:hover` STICKS on a touchscreen — the card stays lifted after the tap. The press is the only feedback a touch reader gets and is deliberately ungated.
...
**Rules**:
- **Three gaps, and they are a hierarchy** — 32 between the card's regions, 24 inside the `Form` group (fields, submit, consent), 16 between fields in a credential stack, with a further 8 of top padding on the account prompt. Taken from the Figma master, where each is bound to `stack/32`, `stack/24`, `stack/16` and `padding/8`. It was a flat 16 throughout until the master was re-spaced on 2026-09-06, which made the submit read as another field and the account prompt read as part of the form.
- **The submit and the consent line belong INSIDE the form group**, not beside it. `.ds-authcard__form` is that group; it is not the `<form>` element, which is the whole card. The name is the master's, so the two can be read side by side.
...
- **It is the LABEL WIDTH that overflows, not the number of tabs.** Measured in a 390px column on 2026-09-06: "Login with Credentials" (185px) + "Login with DARPAN ID" (183px) = 368px of labels in 340px of room, so **TWO** tabs already clip. Keep labels short — the mode, not a sentence about it — pass `overflow` so the row offers the More menu instead of cutting a tab in half, and past three modes use `authSelectorType: "dropdown"` or `"radio"`. Measure at 390: that is `layout/login/content/width`.
