...
**Rules**:
- **`variant="portal"` PUBLISHES FOUR OF DBIM 5.6's SIX ELEMENTS, NOT SIX — open defect, found by audit 2026-09-07.** Sitemap and Help live in `columns`, and the portal variant renders no columns, so both vanish. `relatedLinks` was already special-cased down into the statutory bar for exactly this reason ("moving it for layout reasons must not quietly drop it from one of them") and the same reasoning was never applied to the other two. Nothing is live — the only consumers of `variant="portal"` today are the documentation specimens — but the variant cannot be adopted by a portal until Sitemap and Help render in the statutory bar. The line below, that the statutory half "must stay DBIM-compliant", describes the intent and not yet the behaviour.
