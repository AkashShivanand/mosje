...
- **`sitemap` and `help` draw only on `portal`.** On `website` the content already places them — the estate puts Sitemap in the Support column and Help in the policy row, as dosje.gov.in does — and the clause asks for each element to be present, not present twice. Related Links follows the same rule — a wide row in the working band on `website`, beside the policies on `portal`. An element may change place for layout; it is never dropped from a variant. **On `portal`, never list Sitemap or Help inside `policyLinks`**, or each renders twice.
...
- **Every link in the dosje.gov.in footer is carried, and nothing else is added except what DBIM 5.6 or GIGW 3.0 requires** — Feedback, Related Links (the National Portal of India alone) and the lineage sentence. Checked 2026-09-17; the table is on the documentation page. The one shared destination is the live site's own: "Vision & Mission" opens About Us, as it does there.
