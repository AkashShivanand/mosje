...
**Motion encodes state; it does not decorate.** The seal is **still at rest**, turns at **8s** on hover or focus, and at **4s while the bot is thinking** — so it reports work in progress even when the panel has scrolled the typing dots out of view. It used to turn at 14s on hover only, which is effectively invisible: a one-second hover moves the wordmark about 25°, most of it after the eye has moved on. That was the flask defect again — real motion, below the threshold of being seen. The mascot **floats 3px over 4.5s** wherever the widget is engaged, because the artwork is a legless robot drawn mid-hover and a still picture of a floating thing is subtly wrong. The launcher **arrives once per page load** with a 320ms settle, so it does not pop into a finished layout like a late advert. **There is no sound, and none is planned** — unsolicited audio is hostile on shared terminals and at common service centres, and browsers block it before first interaction anyway, so it would fire inconsistently even if it were wanted.
