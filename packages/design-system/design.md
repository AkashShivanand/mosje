...
- **NOTHING IN THIS PANEL DESTROYS ANYTHING.** The header's ✕ ("Minimise chat") closes and KEEPS the conversation; expand resizes; the footer's **"Start over"** rules the transcript off with a labelled separator (`restartNotice`, "New conversation") and greets again **beneath** it, leaving every earlier turn scrolled up and the panel open.
  ...
  **Both halves shipped together, and neither is sufficient alone.** Moving it without the append leaves a destructive control that people still press *on purpose*, having read "Start over" as "start this question over". Appending without the move leaves a control grouped with the wrong neighbour. **Getting it out of Send's column took TWO attempts, and the first one broke the layout.** Moving it to the head of the note's row cleared the adjacency — and put 101px of button in front of the paragraph, which pushed the disclaimer's left edge **109px inside the composer's**, gave the panel a ragged left margin, and rewrapped the note from two lines to three so the button floated with 16px of dead space above it. Every gate passed; it was caught on review.
  **The footer meta block is a COLUMN**: note at full width, Start over on its own line beneath it, left-aligned. Composer, note and button now share one left edge at 33px, the note is two lines again, and the control is 171px horizontally and 53px vertically from Send. **It costs 24px of panel height** — the line the note gives back pays for most of the row the button takes. The class is still `.ds-chatbot__footer-row` because the Figma parity claim pins that string. The header was the other candidate and lost: an icon beside ✕ would drop the words, and the words are what tell a first-time visitor what the control does. Both facts are pinned in `e2e/chatbot/end-chat.spec.ts` — `does not share a column with Send` and `the footer keeps ONE left edge` — because the gap test passes in BOTH arrangements, so it cannot catch the layout half on its own.
  **The button's BOX aligns, not its glyphs.** `Button` `sm` carries 16px of horizontal padding, so the label starts 17px right of the note's text. Pulling it back with a negative margin would push the control's hover and focus wash outside the panel's content edge, which is worse than the optical indent. Do not "fix" it without solving that.
