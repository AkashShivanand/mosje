...
**Key props**: `items` (`{icon, value, label}[]`), `overlap`, `columns`, `layout`, `ariaLabel`
**Rules**:
...
- **`layout="inline"` once the set reads as a grid rather than a strip.** Three or four centred tiles under a hero are a strip; eight are a grid the eye has to enter eight separate times. Inline sets the mark beside the copy and aligns the cell to the start, so a row scans left to right and the card gives back about a third of its height. It reverts to stacked below 640px — a 48px mark plus a nine-digit figure does not fit a two-up phone cell, and the figure is the one thing that may never wrap, shrink or be covered.
