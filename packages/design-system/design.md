<!--
  ...
  Last reviewed: 2026-09-15 · System version: v0.65.0 (`SiteFooter` OWNS NO WIDTH, AND FIGMA AND CODE
  ...
  from `brand-glyph.tsx`. A Code Connect template now maps the master.)
...
- **NEVER pass a background through `className`.** Colour binds to the mode-aware `--sa-color-primaryScale-*` family — ground = rung 800, hairlines = rung 600 — so the footer repaints for `blue`, `navy`, `dbim` and the five DBIM hues with no work at the call site. In `dbim` the ground resolves to **#162F6A**, DBIM's own published Blue shade 1, satisfying **[DBIM 5.6]** by construction.
...
- **FIGMA PARITY: `Site Footer` on the library's `Footer` page — Variant (Website|Portal) x Breakpoint (Desktop|Tablet|Mobile), six variants, zero unbound values, Code Connect template `site-footer.figma.ts`.** Re-synced 2026-09-15 by measuring every landmark in both at 1440, 768 and 375: they agree within 8px, the residue being text metrics and the live visit counter. That pass moved the code to the container ladder and the 28px list rhythm, and moved the master to the code's 14px policy links, 4px social gap, 64x28 credit marks, 96ch lineage measure, 40px stacked gaps, a fluid portal, Desktop XL mode on the 1440 variants, and the five social glyphs rebuilt from `brand-glyph.tsx`'s own paths and formula. What stays recorded on the page's Component record: **NeGD has no inverse mark** (needs artwork); **Footer - Bottom Strip** survives for ten `PortalLoginTemplate` instances; and four values Figma has no unit for — the `color-mix` chip ground and zone rule (drawn as the `primaryScale/700` and `primaryScale/600` rungs they replace, which match in Blue), the 55% inline-navs hairline (paint opacity), and the 96ch measure (a 659px max width). The website's runtime right-wall gutter is not drawn.
