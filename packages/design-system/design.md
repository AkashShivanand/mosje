...
- **The mark loses its chip in `extended`, and that is what pays for the bigger figure.** A 56px peach tile above a 20px figure is survivable only while the figure is quiet; beside a 32px one it is a second claim on the same attention. As a bare 24px glyph it is still scannable and no longer competing — and because the chip was *taller* than the copy it introduced, an extended card is **shorter** than a compact one holding the same eight items (208px against 232 for NMBA's counters, with figures 60% larger).
