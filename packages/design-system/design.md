...
- **Figma** (Carousel page, rebuilt 22 Sep 2026): `Carousel` (60 variants — Device Desktop/Tablet/Mobile × Slides 5/9/1 × Autoplay × Current, the five live banners in a Smart Animate track, prototype-wired: 7s timer, arrows, dots, Pause/Play); `Carousel / Controls` (42 — Placement × Autoplay Off/Running/Paused × Pagination Dots/Counter × Current 1–6, `Dot 3`–`Dot 6` booleans); parts `Arrow`, `Rotation`, `Dot`, `Counter`, `Slide` with Default/Hover/Pressed/Focus and `Size=Touch` (44px, `target/comfortable`). The nine deprecated carousel masters were deleted, not reused: they baked slogans into the picture.
