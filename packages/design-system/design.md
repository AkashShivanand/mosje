...
**Opening the launcher changes the CENTRE of the seal, not the whole mark.** The band around the disc is 12px because it *carries the wordmark* — at rest that width is earned, there is bilingual type running around it. Fading the whole mark out and a plain disc in left that same band holding nothing: a thick empty white stroke around a small blue circle, reading as a sticker outline rather than a ring. **A band is only a ring if something is on it.** So the seal never leaves; the wordmark, the disc, the silhouette and every measurement stay, and only the middle changes, robot to close glyph. Nothing about the geometry moves, so there is nothing to look wrong. Note the mascot is faded by opacity and filter **only** — it carries the float, which owns its `transform`.
