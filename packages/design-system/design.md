...
- **`bleed` only where the bar reaches the viewport.** Above 1440px the plinth used to run out past the bar's own edge and paint the whole page margin navy — the frame's treatment, and the frame draws the bar edge to edge. The estate's one horizontal ticker sits inside `.sa-container`, so it was bled off the left of the page and inset on the right, under a fact card that is symmetrically placed. It is opt-in now: pass it when the bar genuinely spans the viewport, leave it off in a content column. Below 1440 nothing changes — the plinth was always flush with the bar's own leading edge, and still is.
