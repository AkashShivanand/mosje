...
**The Figma property model was wrong, and is fixed (2026-08-25)**: the set exposes exactly four properties — `Label` (text, default `"Latest Updates"`) and the axes `Breakpoint`, `Direction`, `Motion`. The parent's single text property was **named `Title` while it drove the plinth label**, and `ticker.figma.ts` read five more — `Title`, `Description`, `Show Description`, `Action Label`, `Show Action` — that **had never existed on the set**; they are properties of the nested parts and were never exposed. So the generated snippet put the strip's own name into the first notice's headline and left `label` blank. The template now reads only `Label` and `Direction`, and seeds a realistic pair for `items`, because `items` is data and one frame draws one message. The variants also carried a label of "What's New" and an action of "View All" against the code defaults `"Latest Updates"` and `"View All Updates"` — the action's own nested default was already correct, so those 16 were stale overrides. `npm run check:code-connect` gates the parity.
