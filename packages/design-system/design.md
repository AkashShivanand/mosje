...
**Props**: `variant`, `appearance`, `size`, `iconLeft`, `iconRight`, `disabled`, `href` (renders as `<a>`), `linkAs` (the app's router link — pass `next/link` with every internal `href`)  
**Rules**:
...
- **`href` + `linkAs={Link}` for an internal destination.** Without `linkAs` the link form is a bare `<a>` and every click reloads the whole document; 37 call sites did until 2026-09-22. `check:link-as` now demands it wherever a Button carries an `href` (`linkAs-gate(href-only)` on the interface). A disabled, external, scheme or `#` href — and a download or new-tab link — stays a plain `<a>` whatever is passed, decided by the same `navLinkRoutes` rule as every other `linkAs` component.
- **A link that looks like a button is `<Button href linkAs>`, not `<Link className={buttonClasses()}>`** — the helper gives the colours but not the icon slot, so a glyph inside it misses the icon-side padding and the disabled handling.
