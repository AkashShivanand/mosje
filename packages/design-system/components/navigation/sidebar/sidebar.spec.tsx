...
describe("SidebarNav", () => {
  ...
  it("renders three levels as nested lists and marks the current page at level 3", () => {
    ...
    // two tints — the level-1 holder and the page — and the level-2 group on the way in ink
    expect(out.match(/is-active/g)?.length).toBe(2);
    expect(out.match(/is-ancestor/g)?.length).toBe(1);
  });

  it("draws the path to the current page with the connector's own geometry", () => {
    // first level-2 row: list padding 4, row centre 22 → y 26; trunk at 24, the elbow leaves it at 20 and meets the arm at 26, reaching the pill at 40
    expect(activePathD(24, 26)).toBe("M 24 0 V 20 Q 24 26 30 26 H 40");
    // a level-3 list is nested inside its level-2 entry's 40px indent, so its trunk is at 8 in that list
    // (48 in the rail, under the parent label's first letter) and the arm reaches the pill at 24 (64 in the rail); third row: 4 + 96 + 22
    expect(activePathD(8, 122)).toBe("M 8 0 V 116 Q 8 122 14 122 H 24");
  });
