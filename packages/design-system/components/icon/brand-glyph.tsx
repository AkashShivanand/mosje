 ...
 *
 * 22 rather than 24: vendor artwork is drawn to the edge of its own box, so
 * five full-bleed marks in a row touch each other's boxes and the rail reads
 * as crowded. Two units of air is the difference between a set and a pile.
 */
const OPTICAL_SIZE = 22;
 ...
 *
 *    The `optical` values are a HALF-correction toward equal ink: full
 *    correction would shrink YouTube's solid slab until it read as small, and
 *    inflate Instagram's hairline camera until it read as large. The eye judges
 *    a mark by both extent and weight, so the correction splits the difference.
 *
 * Result: ink spread 2.37× → 1.51×, with every mark still the vendor's own.
 * That is about as tight as officially-correct artwork allows; the residue is
 * YouTube, which is intrinsically a filled rectangle.
 *
 ...
 * `optical: 1`, render it beside the others at 4× and adjust in steps of 0.02
 * until it sits in the row. `box` and `path` must be updated together — a stale
 * box misplaces the mark visibly, which is the intended failure mode.
 */
const GLYPHS: Record<BrandGlyphName, BrandGlyphDef> = {
  ...
  facebook: {
    ...
    /* A letterform reads smaller than a square mark of the same height. */
    optical: 1.05,
  },
  x: {
    title: "X",
  ...
  instagram: {
    ...
    box: [0, 0, 24, 24],
    /* An outline encloses more area than it inks; pulled back a touch. */
    optical: 0.95,
  },
  youtube: {
    title: "YouTube",
    ...
    /* The one solid slab in the set, and the heaviest mark by a wide margin. */
    optical: 0.86,
  },
  whatsapp: {
    title: "WhatsApp",
 ...
 * the thing a row of five logos actually needs and that no vendor supplies: a
 * single optical size, so the set reads as a set. See OPTICAL NORMALISATION
 * above for the measurements and the tuning rule.
 *
