...
| `Metric Card` | a narrow arrangement | ✅ **16 Sep — nothing to change.** On a phone the portal draws every tile at the card's full width: 327 wide on the NGO dashboard and on Attendance, 110 tall, badge at 40. The master is already fluid at that width (0 overflow at 327). The code's two narrower rungs — the badge drops to 32 in a tile under 240 and is not drawn under 200 — are a container query Figma cannot reproduce, so a designer drawing two tiles side by side turns `Show icon` off. **Finding, not fixed:** a half-width tile carrying a status chip squeezes its label to about 53px in the build as well as in Figma (measured at 165: label 53×40, chip 58 beside it); the chip should probably move under the label in a narrow tile, which is a code change |
| `Event List / Row` | a narrow arrangement | ✅ **16 Sep — no new variant: the code has none.** The item is a three-column grid (`auto 1fr auto`) whose middle column simply narrows and wraps. All 20 variants now do the same — the body fills, the subject text fills with a 120 minimum so it moves under the action when both no longer fit, the date flag wraps with its own gap, the attribution and the quoted note fill and wrap. **Before, at 291 a row ran 189px out of its own card**; now 0. Every master is unchanged at its own size. Drawn as a sixth arrangement cell on `Event List — Documentation`. The Component record had been overlapping section 1 by 84px; the documentation and the record were moved up so the record ends 160 above it. **Waits on a publish** |
| `File List / Row` | a narrow arrangement | ✅ **16 Sep — no new variant: the code has none.** The row is `flex-wrap: wrap`, the details take a 14rem (224px) basis, the actions `margin-inline-start: auto`. All 4 variants now wrap like `List Row` — `WRAP`, line gap bound to `stack/12`, details `FILL` with a 224 minimum, actions `HUG`, aligned to the end; every text fills and wraps; the progress bar fills the details column. **Measured against the build:** actions under the details, 16 from the right edge and 12 below. **Before, at 291 the actions ran 47px out of the row.** The vertical padding was 16 against the code's 12 (`padding: 12px 16px`) and is now bound to `padding/12`, so every master is **8px shorter** (Attached 88 → 80). Drawn as a sixth arrangement cell on `File List — Documentation`. **Waits on a publish** |
