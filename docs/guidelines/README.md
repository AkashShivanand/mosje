...
| 4 | UX4G tokens are prefixed `--ux4g-*` | Canonical tokens are `--sa-*` (SAMAVESH) | One namespace per estate. A `--ux4g-*` mapping is generated for conformance measurement only (`tools/ux4g-conformance/parity.generated.css`, a build output read by `measure.mjs`); it is not shipped and not for authoring. |
