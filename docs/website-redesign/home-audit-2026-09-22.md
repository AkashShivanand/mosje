...
**Carousel completed, same day.** Code: a single slide now renders alone (it used to draw arrows, a dot and Pause that did nothing), and a long rotating set shows its timer under the counter. Figma: the component is rebuilt as parts (Arrow, Rotation, Dot, Counter, Slide — every state, 44px touch sizes), Controls (42 variants) and a full `Carousel` (60 variants across desktop, tablet and phone, 5 / 9 / 1 slides) carrying the live banners and 279 prototype connections. The deprecated slides and controls (21 masters) were deleted rather than reused — their slides baked marketing slogans into the image. Open: click through the prototype once in presentation mode to confirm a hovered control still navigates; refresh the Index card preview.

**Carousel audit and variant optimisation, same day.** Coverage gaps closed: autoplay off with dots, and autoplay on with the counter. Variants 175 → 127. Controls 42 → 4 (Placement × Pagination; the current dot and Pause/Play are set on the nested parts, autoplay on/off is a `Rotation` switch). The full Carousel 60 → 58 while covering more: Desktop and Tablet merge into one resizable `Layout=Pill` (3:1 locked, pill pinned to the corner), autoplay off is `Rotation=Stopped` with the switch off. Parts 73 → 65: hover removed from touch-size controls (it never fires on touch) and from the current dot (identical to default).

**Animated timer in the Figma prototype, same day.** The running dot and the counter's hairline were drawn part-filled and never moved. Each is now an interactive part: it starts empty and fills to full over 7 seconds, linearly, in step with the carousel's 7-second advance — as the code draws it. The two "focused while running" dot states were removed: in code, focus holds the rotation, so they cannot occur. 129 variants.
