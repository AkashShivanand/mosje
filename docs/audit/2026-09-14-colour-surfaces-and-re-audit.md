...
should be. **None is applied in this change** — each moves published values and needs a decision.

**1 · The sidebar route tint is invisible in Navy.** `bg/brand/primary/base` under Navy is `#F7FAFF`,
...
**2 · Selected differs from hover by hue, not by lightness.** Selected sits 1.1 L\* (Blue) and 1.8 L\*
...
**3 · The lightest tint is a different lightness in every family.** Rung 50: error 92.7, Blue primary
...
- **`build/brand-ramps.mjs` is not idempotent.** Running it deletes `color/transparent/secondary/0`, the
  alpha tier the sidebar identity wash binds. It was not run to write `semantic.json` in this change.
