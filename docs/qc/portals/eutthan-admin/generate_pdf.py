#!/usr/bin/env python3
"""Generate the eUtthan Admin Design QC PDF from audit-master.json.
HTML (new component-based design) -> Chrome headless --print-to-pdf.
...
def panel(img_rel, box, pins, side):
    ...
    if not img_rel:
        return ('<div class="crop empty">No Figma design — frame collapsed'
                '<div class="empty-sub">live-only, no side-by-side comparison</div></div>')
    x1,y1,x2,y2 = box
    cw, ch = (x2-x1), (y2-y1)
    scale = PANEL_W / cw
...
def board(screen, section, sec_findings):
    ...
    if not single:
        panels += (f'<div class="pwrap"><div class="plabel design"><b>DESIGN</b> Figma intent</div>'
                   f'{panel(figma_img, box, fig_pins, "figma")}</div>')
    panels += (f'<div class="pwrap"><div class="plabel build"><b>BUILD</b> '
               f'Live build</div>'
               f'{panel(live_img, box, live_pins, "live")}</div>')
    return (f'<div class="board">'
            ...
            f'<div class="bfoot"><span>MoSJE Design QC — eUtthan Admin Portal · {esc(am.get("generated",""))}</span>'
...
cover=(f'<section class="cover"><div class="kicker">MINISTRY OF SOCIAL JUSTICE &amp; EMPOWERMENT &nbsp;·&nbsp; DESIGN QC</div>'
       f'<h1>eUtthan Admin Portal</h1><div class="sub">Design Quality Audit — Figma design intent vs. live build</div>'
...
doc=(f'<!doctype html><html><head><meta charset="utf-8">'
     ...
     f'<style>{CSS}</style></head><body>{cover}{screens_html}</body></html>')

html_path=os.path.join(BASE,"report-generated.html")
pdf_path=os.path.join(BASE,"eUtthan-Admin-Design-QC-Report.pdf")
