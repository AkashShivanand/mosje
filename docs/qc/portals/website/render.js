...
(async () => {
  ...
  async function renderSection(i) {
    ...
    // Every board image must be DECODED before the page prints. 'load' fires when the bytes have
    // arrived, not when the bitmap is ready, and a page that prints early comes out with white
    // boards and the marks floating on nothing — which is worse than no board at all.
    ...
    const undecoded = await page.evaluate(() =>
      Array.from(document.images).filter((im) => !im.complete || im.naturalWidth === 0)
        .map((im) => im.src.split("/").pop()));
    if (undecoded.length) throw new Error("images not decoded: " + undecoded.join(", ").slice(0, 120));
