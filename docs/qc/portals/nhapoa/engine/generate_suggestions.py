...
Reads suggestions.json next to this script. Two accepted shapes:
  ...
  2. An object with meta + items (preferred):         { "portal": "...", "generated": "YYYY-MM-DD",
                                                        "items": [ {item}, ... ] }
Item: {id, group:"A"|"B"|"C", type:"Decide"|"Design"|"Propose", title, observed, recommendation,
       board?: {figmaImg?, liveImg?, box:[x1,y1,x2,y2], figmaPins?:[[n,x,y]], livePins?:[[n,x,y]]}}
Groups: A — program-level token decisions · B — updates to existing Figma frames ·
        C — screens built live with no design (proposals needed).
...
groups = {"A": "A — Program-level token decisions",
          "B": "B — Updates to existing Figma frames",
          "C": "C — Screens built live with no design (proposals needed)"}
bodyparts = [cover]; n = 0
...
json.dump({"css": CSS, "sections": ["".join(bodyparts)], "width": 1100, "out": out},
