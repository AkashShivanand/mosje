/**
 * States / UTs and their districts, for the cascading State → District selects the live
 * E-Anudaan wizards use (AVYAY step 2 & 3, SMILE step 2).
 *
 * The live portal drives these from the LGD (Local Government Directory) master. This is a
 * transcription for the clone: the STATE list is the live list verbatim (36 entries, in the
 * live alphabetical order, note that Uttarakhand precedes Uttar Pradesh exactly as observed).
 * District lists are reference data — replace with the LGD master when the API is wired.
 */
...
/** North-Eastern and Himalayan States — 95% central share under the AVYAY cost norms. */
export const NE_HIMALAYAN_STATES: readonly string[] = [
  "Arunachal Pradesh",
  "Assam",
  "Himachal Pradesh",
  "Jammu and Kashmir",
...
export const DISTRICTS: Record<string, readonly string[]> = {
  ...
  Punjab: [
    ...
    "Pathankot", "Patiala", "Rupnagar", "Sahibzada Ajit Singh Nagar", "Sangrur", "Shaheed Bhagat Singh Nagar",
...
/** Districts for a state, or an empty list so the dependent select renders disabled. */
export function districtsOf(state: string | undefined): readonly string[] {
  if (!state) return [];
  return DISTRICTS[state] ?? [];
}

/**
 * 7th-CPC HRA city classification the AVYAY cost norms key off. X = the eight metros,
 * Y = the listed 5–50 lakh cities, Z = everything else (the live default).
 */
const X_DISTRICTS = new Set([
  "Ahmedabad", "Bengaluru Urban", "Chennai", "Delhi", "New Delhi", "Central Delhi", "East Delhi",
  "North Delhi", "North East Delhi", "North West Delhi", "Shahdara", "South Delhi", "South East Delhi",
  "South West Delhi", "West Delhi", "Hyderabad", "Kolkata", "Mumbai City", "Mumbai Suburban", "Pune",
]);

const Y_DISTRICTS = new Set([
  "Agra", "Ajmer", "Aligarh", "Amravati", "Amritsar", "Asansol", "Aurangabad", "Bareilly", "Belagavi",
  "Bhavnagar", "Bhiwandi", "Bhopal", "Bhubaneswar", "Bikaner", "Bilaspur", "Chandigarh", "Coimbatore",
  "Cuttack", "Dehradun", "Dhanbad", "Durg", "Erode", "Faridabad", "Firozabad", "Ghaziabad", "Gorakhpur",
  "Guntur", "Gurugram", "Guwahati", "Gwalior", "Hubballi", "Indore", "Jabalpur", "Jaipur", "Jalandhar",
  "Jammu", "Jamnagar", "Jamshedpur", "Jhansi", "Jodhpur", "Kannur", "Kanpur Nagar", "Kakinada",
  "Kochi", "Kolhapur", "Kollam", "Kota", "Kozhikode", "Kurnool", "Lucknow", "Ludhiana", "Madurai",
  "Malappuram", "Mangaluru", "Mathura", "Meerut", "Moradabad", "Mysuru", "Nagpur", "Nanded", "Nashik",
  "Nellore", "Patna", "Prayagraj", "Puducherry", "Raipur", "Rajkot", "Ranchi", "Saharanpur", "Salem",
  "Sangli", "Siliguri", "Solapur", "Srinagar", "Surat", "Thiruvananthapuram", "Thrissur", "Tiruchirappalli",
  "Tiruppur", "Ujjain", "Vadodara", "Varanasi", "Vasai-Virar", "Vijayawada", "Visakhapatnam", "Warangal Urban",
]);

export const CITY_CATEGORIES = [
  "X — metro (population 50 lakh and above)",
  "Y — city of 5 to 50 lakh",
  "Z — below 5 lakh, or a rural location",
] as const;

/** The tier the live form fills in once a district is chosen. */
export function cityCategoryFor(district: string | undefined): string {
  if (!district) return "";
  if (X_DISTRICTS.has(district)) return CITY_CATEGORIES[0];
