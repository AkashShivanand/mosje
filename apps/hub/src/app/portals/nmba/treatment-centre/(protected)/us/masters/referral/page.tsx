...
import { REFERRED_BY as INITIAL_REFERRED_BY } from "@/lib/nmba/treatment-centre/master-data";
...
export default function USReferralMasterPage() {
  const { toast } = useToast();
  const [items, setItems] = React.useState<MasterItem[]>(
    INITIAL_REFERRED_BY.map((d, i) => ({ sno: i + 1, label: d.label, value: d.value }))
  );
  ...
  const handleDelete = (item: MasterItem) => {
    if (confirm(`Are you sure you want to delete "${item.label}"?`)) {
      ...
      toast("Referral source deleted successfully.", "warning");
    }
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editingItem) {
      ...
      toast("Referral source updated successfully.", "success");
    } else {
      if (items.some((i) => i.value === value)) {
        toast("Referral source code (value) must be unique.", "error");
        return;
      }
      setItems((prev) => [...prev, { sno: prev.length + 1, label, value }]);
      toast("Referral source added successfully.", "success");
    }
    setModalOpen(false);
  };
  ...
  const columns: ColumnDef<MasterItem>[] = [
    { key: "sno", header: "S.No" },
    { key: "label", header: "Referral Source" },
    { key: "value", header: "Source Code (Value)" },
    {
      key: "actions",
      header: "Action",
      render: (r) => (
  ...
  return (
    <>
      <TCListPage
        title="Referral Source Master"
        columns={columns}
        data={items}
        searchKeys={["label", "value"]}
        fileName="referral-source-master"
        action={
          <button
            type="button"
          ...
          >
            <Plus className="h-4 w-4" aria-hidden /> Add New Referral Source
          </button>
        }
      />
      ...
      <Modal
        ...
        title={editingItem ? "Edit Referral Source details" : "Add New Referral Source"}
        footer={
          <>
            <Button type="button" appearance="outlined" onClick={() => setModalOpen(false)}>Cancel</Button>
            <Button type="submit" form="referral-form">Save</Button>
          </>
        }
      >
        <form id="referral-form" onSubmit={handleSave} className="flex flex-col gap-4">
          <FormField label="Referral Source" required>
            {(c) => <Input {...c} value={label} onChange={(e) => setLabel(e.target.value)} placeholder="e.g. Self" />}
          </FormField>
          <FormField label="Source Code (Value)" required>
            {(c) => (
              <Input
                {...c}
                value={value}
