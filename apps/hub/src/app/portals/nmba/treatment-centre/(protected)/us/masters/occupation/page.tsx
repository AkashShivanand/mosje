...
import { OCCUPATION as INITIAL_OCCUPATION } from "@/lib/nmba/treatment-centre/master-data";
...
export default function USOccupationMasterPage() {
  const { toast } = useToast();
  const [items, setItems] = React.useState<MasterItem[]>(
    INITIAL_OCCUPATION.map((d, i) => ({ sno: i + 1, label: d.label, value: d.value }))
  );
  ...
  const handleDelete = (item: MasterItem) => {
    if (confirm(`Are you sure you want to delete "${item.label}"?`)) {
      ...
      toast("Occupation deleted successfully.", "warning");
    }
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editingItem) {
      ...
      toast("Occupation updated successfully.", "success");
    } else {
      if (items.some((i) => i.value === value)) {
        toast("Occupation code (value) must be unique.", "error");
        return;
      }
      setItems((prev) => [...prev, { sno: prev.length + 1, label, value }]);
      toast("Occupation added successfully.", "success");
    }
    setModalOpen(false);
  };
  ...
  const columns: ColumnDef<MasterItem>[] = [
    { key: "sno", header: "S.No" },
    { key: "label", header: "Occupation Name" },
    { key: "value", header: "Occupation Code (Value)" },
    {
      key: "actions",
      header: "Action",
      render: (r) => (
    ...
    <>
      <TCListPage
        ...
        action={
          ...
          >
            <Plus className="h-4 w-4" aria-hidden /> Add New Occupation
          </button>
        }
      />
      ...
      <Modal
        ...
        title={editingItem ? "Edit Occupation details" : "Add New Occupation"}
        footer={
          <>
            <Button type="button" appearance="outlined" onClick={() => setModalOpen(false)}>Cancel</Button>
            <Button type="submit" form="occupation-form">Save</Button>
          </>
        }
      >
        <form id="occupation-form" onSubmit={handleSave} className="flex flex-col gap-4">
          <FormField label="Occupation Name" required>
            {(c) => <Input {...c} value={label} onChange={(e) => setLabel(e.target.value)} placeholder="e.g. Daily Wage Labourer" />}
          </FormField>
          <FormField label="Occupation Code (Value)" required>
            {(c) => (
              <Input
                {...c}
                value={value}
