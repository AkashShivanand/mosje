...
import { GENDER_MASTER } from "@/lib/nmba/treatment-centre/masters-extra";
...
export default function USGenderMasterPage() {
  const { toast } = useToast();
  const [items, setItems] = React.useState<MasterItem[]>(
    GENDER_MASTER.map((d, i) => ({ sno: i + 1, label: d.label, value: d.value, status: d.status }))
  );
  ...
  const handleDelete = (item: MasterItem) => {
    if (confirm(`Are you sure you want to delete "${item.label}"?`)) {
      ...
      toast("Gender deleted successfully.", "warning");
    }
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editingItem) {
      ...
      if (items.some((i) => i.value === value)) {
        toast("Gender code (value) must be unique.", "error");
        return;
      }
  ...
  const columns: ColumnDef<MasterItem>[] = [
    ...
    { key: "value", header: "Gender Code (Value)" },
    {
      key: "status",
      header: "Status",
      render: (r) => (
    ...
    <>
      <TCListPage
        ...
        action={
          ...
          >
            <Plus className="h-4 w-4" aria-hidden /> Add New Gender
          </button>
        }
      />
      ...
      <Modal
        ...
        title={editingItem ? "Edit Gender details" : "Add New Gender"}
        footer={
          <>
            <Button type="button" appearance="outlined" onClick={() => setModalOpen(false)}>Cancel</Button>
            <Button type="submit" form="gender-form">Save</Button>
          </>
        }
      >
        <form id="gender-form" onSubmit={handleSave} className="flex flex-col gap-4">
          <FormField label="Gender" required>
            {(c) => <Input {...c} value={label} onChange={(e) => setLabel(e.target.value)} placeholder="e.g. Male" />}
          </FormField>
          <FormField label="Gender Code (Value)" required>
            {(c) => (
              <Input
                {...c}
                value={value}
