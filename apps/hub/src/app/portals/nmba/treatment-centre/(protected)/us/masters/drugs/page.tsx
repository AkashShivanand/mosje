...
import { DRUGS as INITIAL_DRUGS } from "@/lib/nmba/treatment-centre/master-data";
...
export default function USDrugsMasterPage() {
  const { toast } = useToast();
  const [items, setItems] = React.useState<MasterItem[]>(
    INITIAL_DRUGS.map((d, i) => ({ sno: i + 1, label: d.label, value: d.value }))
  );
  ...
  const handleDelete = (item: MasterItem) => {
    if (confirm(`Are you sure you want to delete "${item.label}"?`)) {
      ...
      toast("Drug deleted successfully.", "warning");
    }
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editingItem) {
      ...
      toast("Drug updated successfully.", "success");
    } else {
      if (items.some((i) => i.value === value)) {
        toast("Drug code (value) must be unique.", "error");
        return;
      }
      setItems((prev) => [...prev, { sno: prev.length + 1, label, value }]);
      toast("Drug added successfully.", "success");
    }
    setModalOpen(false);
  };
  ...
  const columns: ColumnDef<MasterItem>[] = [
    { key: "sno", header: "S.No" },
    { key: "label", header: "Substance / Drug Name" },
    { key: "value", header: "Substance Code (Value)" },
    {
      key: "actions",
      header: "Action",
      render: (r) => (
  ...
  return (
    <>
      <TCListPage
        title="Drug / Substance Master"
        columns={columns}
        data={items}
        searchKeys={["label", "value"]}
        fileName="drugs-master"
        action={
          <button
            type="button"
          ...
          >
            <Plus className="h-4 w-4" aria-hidden /> Add New Drug
          </button>
        }
      />
      ...
      <Modal
        ...
        title={editingItem ? "Edit Drug details" : "Add New Drug"}
        footer={
          <>
            <Button type="button" appearance="outlined" onClick={() => setModalOpen(false)}>Cancel</Button>
            <Button type="submit" form="drug-form">Save</Button>
          </>
        }
      >
        <form id="drug-form" onSubmit={handleSave} className="flex flex-col gap-4">
          <FormField label="Drug Name" required>
            {(c) => <Input {...c} value={label} onChange={(e) => setLabel(e.target.value)} placeholder="e.g. Heroin" />}
          </FormField>
          <FormField label="Drug Code (Value)" required>
            {(c) => (
              <Input
                {...c}
                value={value}
