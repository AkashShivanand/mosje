"use client";

import * as React from "react";
import { useRouter } from "next/navigation";
import { Alert, Button, Checkbox, FormField, Icon, Input, MediaUpload, RadioGroup, Select, Textarea, type SelectOption, type StepperStep } from "@mosje/design-system";
import { useToast } from "@/components/nmba/toast";
import { useTCSession } from "@/lib/nmba/treatment-centre/session-context";
import { useTCStore } from "@/lib/nmba/treatment-centre/store";
import { FormSection, FormCard } from "@/components/nmba/treatment-centre/tc-form";
import { Wizard, ReviewItem, ReviewSection } from "@/components/nmba/treatment-centre/tc-wizard";
import type { DrugUseRow, Patient } from "@/lib/nmba/treatment-centre/types";
import {
  GENDERS,
  STATES,
  PLACE_OF_RESIDENCE,
  MARITAL_STATUS,
  LIVING_ARRANGEMENTS,
  EDUCATION,
  OCCUPATION,
  EMPLOYMENT,
  INCOME,
  CATEGORY,
  GOVERNMENT_ID,
  DRUGS,
  INITIATION_REASONS,
  YES_NO,
  YES_NO_NR,
  ASSIST_SCORE_ALCOHOL,
  ASSIST_SCORE_OTHER,
  SEXUAL_PRACTICES,
  TEST_RESULT,
  TREATMENT_TAKEN,
  MONEY_SOURCE_OPTIONS,
  MOTIVATION_STAGES,
  REFERRED_BY,
  PROVISIONAL_DIAGNOSIS,
  districtsForState,
} from "@/lib/nmba/treatment-centre/master-data";

function labelOf(options: SelectOption[], value: string): string {
  return options.find((o) => o.value === value)?.label ?? value;
}

/** "Any Other" sentinel values that reveal a free-text "specify" field. */
const LIVING_OTHER = "5";
const EMPLOYMENT_OTHER = "6";
const REFERRAL_OTHER = "15";

/** Rows of the legal / criminal-history matrix (Ever + Last-month Yes/No each). */
const LEGAL_ROWS: Array<{ key: string; label: string }> = [
  { key: "selling", label: "For Selling" },
  { key: "possession", label: "For possession" },
  { key: "unruly", label: "For unruly behaviour under the influence" },
  { key: "otherCrime", label: "Any other crime (e.g. stealing)" },
];

const emptyDrugRow: DrugUseRow = {
  drug: "",
  ageOfFirstUse: "",
  reason: "",
  usedLast3Months: "",
  dailyUse: "",
  durationMonths: "",
};

const STEPS: StepperStep[] = [
  { label: "Patient Identity", description: "Photo, name & address" },
  { label: "Socio-economic & ID", description: "Background & ID" },
  { label: "Substance Use", description: "Drug & behaviour" },
  { label: "Assessment", description: "Scores & diagnosis" },
  { label: "Review", description: "Confirm & submit" },
];

// Required field keys per input step (mirrors the legacy form's mandatory `*` markers).
const STEP_REQUIRED: Record<number, string[]> = {
  0: [
    "dateOfAdmission",
    "name",
    "gender",
    "age",
    "currentAddress",
    "permanentAddress",
    "state",
    "district",
    "placeOfResidence",
    "contactNumber",
  ],
  1: [
    "maritalStatus",
    "livingArrangements",
    "education",
    "occupation",
    "employment",
    "income",
    "category",
    "governmentId",
    "governmentIdNumber",
  ],
  3: [
    "assistAlcohol",
    "assistOther",
    "previousTreatment",
    "sourceOfReferral",
    "everHospitalized",
    "avgExpenditure",
    "everApprehended",
    "familyHistory",
    "daysSinceLast",
    "motivation",
    "provisionalDiagnosis",
  ],
};

const FIELD_LABELS: Record<string, string> = {
  patientImage: "Patient Photo",
  dateOfAdmission: "Date of Admission",
  name: "Name of the Patient",
  gender: "Gender",
  age: "Age",
  currentAddress: "Current Address",
  permanentAddress: "Permanent Address",
  state: "State",
  district: "District",
  placeOfResidence: "Place of Residence",
  contactNumber: "Contact Number",
  maritalStatus: "Marital Status",
  livingArrangements: "Living Arrangements",
  livingArrangementsOther: "Living Arrangements (specify)",
  education: "Educational Status",
  occupation: "Occupational Status",
  employment: "Employment Status",
  employmentOther: "Employment Status (specify)",
  income: "Income",
  category: "Category",
  governmentId: "Government ID",
  governmentIdNumber: "Government ID Number",
  treatmentTaken: "If yes, treatment taken from",
  sourceOfReferralOther: "Source of Referral (specify)",
  moneySource: "Source of Money for Drug Use",
  drug: "At least one drug-use row",
  ivEver: "Intravenous Drug Use Ever",
  iv3m: "Intravenous Drug Use (last 3 months)",
  sharingEver: "Sharing Needles Ever",
  sharing3m: "Sharing Needles (last 3 months)",
  sexualPractices: "Sexual Practices",
  hcvHistory: "HCV testing — Test History",
  hbvHistory: "HBV testing — Test History",
  assistAlcohol: "ASSIST Score For Alcohol Use",
  assistOther: "ASSIST Score For Other Drugs",
  previousTreatment: "Previous Treatment for Substance use",
  sourceOfReferral: "Source of Referral",
  everHospitalized: "Ever hospitalized",
  avgExpenditure: "Average Daily Expenditure",
  everApprehended: "Ever apprehended by police",
  familyHistory: "Family history of substance use",
  daysSinceLast: "Days since last consumed",
  motivation: "Patient's Motivation at admission",
  provisionalDiagnosis: "Provisional Diagnosis",
};

const DRAFT_KEY = "nmba-irca-reg-draft";
const todayIso = () => new Date().toISOString().slice(0, 10);

const INITIAL_FIELDS = {
  dateOfAdmission: "",
  name: "",
  gender: "",
  age: "",
  currentAddress: "",
  permanentAddress: "",
  state: "",
  district: "",
  placeOfResidence: "",
  contactNumber: "",
  maritalStatus: "",
  livingArrangements: "",
  livingArrangementsOther: "",
  education: "",
  occupation: "",
  employment: "",
  employmentOther: "",
  income: "",
  category: "",
  governmentId: "",
  governmentIdNumber: "",
  // Substance use (behaviour)
  ivEver: "",
  iv3m: "",
  sharingEver: "",
  sharing3m: "",
  sexualPractices: "",
  hcvHistory: "",
  hcvResult: "",
  hbvHistory: "",
  hbvResult: "",
  // Assessment
  assistAlcohol: "",
  assistOther: "",
  previousTreatment: "",
  treatmentTaken: "",
  sourceOfReferral: "",
  sourceOfReferralOther: "",
  everHospitalized: "",
  avgExpenditure: "",
  everApprehended: "",
  // Legal / criminal history (each Yes/No)
  sellingEver: "",
  sellingLast: "",
  possessionEver: "",
  possessionLast: "",
  unrulyEver: "",
  unrulyLast: "",
  otherCrimeEver: "",
  otherCrimeLast: "",
  otherCrimeSpecify: "",
  familyHistory: "",
  daysSinceLast: "",
  motivation: "",
  provisionalDiagnosis: "",
};

// Named fields resolve to `string` (not `string | undefined`) on dot access; the
// intersected index signature keeps dynamic bracket access (`f[key]`) working for
// validation loops driven by string arrays.
type IrcaFields = typeof INITIAL_FIELDS & Record<string, string>;

export default function IrcaRegisterPage() {
  const router = useRouter();
  const { toast } = useToast();
  const session = useTCSession();
  const store = useTCStore();

  const [f, setF] = React.useState<IrcaFields>(INITIAL_FIELDS);
  const [sameAddress, setSameAddress] = React.useState(false);
  const [photo, setPhoto] = React.useState<{ url: string; name: string } | null>(null);
  const [moneySource, setMoneySource] = React.useState<string[]>([]);
  const keyRef = React.useRef(0);
  const [drugRows, setDrugRows] = React.useState<Array<DrugUseRow & { _key: string }>>([
    { ...emptyDrugRow, _key: "row-0" },
  ]);
  const [errors, setErrors] = React.useState<Set<string>>(new Set());
  const [submitError, setSubmitError] = React.useState("");
  const [step, setStep] = React.useState(0);
  const [draftRestored, setDraftRestored] = React.useState(false);
  const errorSummaryRef = React.useRef<HTMLDivElement>(null);
  const submittingRef = React.useRef(false);
  const drugSectionId = React.useId();

  // ---- Draft restore (session-scoped, text fields only — photo is re-attached) ----
  React.useEffect(() => {
    try {
      const raw = sessionStorage.getItem(DRAFT_KEY);
      if (!raw) return;
      const draft = JSON.parse(raw) as {
        f?: Record<string, string>;
        drugRows?: Array<DrugUseRow & { _key: string }>;
        sameAddress?: boolean;
        moneySource?: string[];
      };
      const hasData =
        (draft.f && Object.values(draft.f).some(Boolean)) ||
        (draft.drugRows && draft.drugRows.some((r) => r.drug));
      if (!hasData) return;
      // One-time restore of a saved draft on mount (sync from external storage).
      // eslint-disable-next-line react-hooks/set-state-in-effect
      if (draft.f) setF({ ...INITIAL_FIELDS, ...draft.f });
      if (draft.drugRows?.length) setDrugRows(draft.drugRows);
      if (draft.sameAddress) setSameAddress(true);
      if (draft.moneySource?.length) setMoneySource(draft.moneySource);
      setDraftRestored(true);
    } catch {
      /* ignore malformed draft */
    }
  }, []);

  // ---- Draft autosave ----
  React.useEffect(() => {
    if (submittingRef.current) return;
    const dirty = Object.values(f).some(Boolean) || drugRows.some((r) => r.drug);
    if (!dirty) return;
    try {
      sessionStorage.setItem(DRAFT_KEY, JSON.stringify({ f, drugRows, sameAddress, moneySource }));
    } catch {
      /* storage full / unavailable — non-fatal */
    }
  }, [f, drugRows, sameAddress, moneySource]);

  // ---- Warn before leaving with unsaved data ----
  React.useEffect(() => {
    const dirty =
      !submittingRef.current && (Object.values(f).some(Boolean) || drugRows.some((r) => r.drug) || !!photo);
    if (!dirty) return;
    const handler = (e: BeforeUnloadEvent) => {
      e.preventDefault();
      e.returnValue = "";
    };
    window.addEventListener("beforeunload", handler);
    return () => window.removeEventListener("beforeunload", handler);
  }, [f, drugRows, photo]);

  const set = (key: string) => (value: string) => setF((prev) => ({ ...prev, [key]: value }));

  const toggleMoneySource = (value: string) =>
    setMoneySource((prev) => (prev.includes(value) ? prev.filter((v) => v !== value) : [...prev, value]));

  const onSameAddress = (checked: boolean) => {
    setSameAddress(checked);
    if (checked) setF((prev) => ({ ...prev, permanentAddress: prev.currentAddress }));
  };

  const updateDrugRow = (i: number, patch: Partial<DrugUseRow>) =>
    setDrugRows((prev) => prev.map((row, idx) => (idx === i ? { ...row, ...patch } : row)));

  const districtOptions = f.state ? districtsForState(f.state) : [];

  /** Missing/invalid required fields for a given input step. */
  const missingForStep = React.useCallback(
    (s: number): Set<string> => {
      const missing = new Set<string>();
      if (s === 0) {
        for (const key of STEP_REQUIRED[0] ?? []) if (!f[key]) missing.add(key);
        if (f.contactNumber && f.contactNumber.length !== 10) missing.add("contactNumber");
        if (f.age) {
          const n = Number(f.age);
          if (!Number.isInteger(n) || n < 1 || n > 120) missing.add("age");
        }
        if (f.dateOfAdmission && f.dateOfAdmission > todayIso()) missing.add("dateOfAdmission");
      }
      if (s === 1) {
        for (const key of STEP_REQUIRED[1] ?? []) if (!f[key]) missing.add(key);
        if (f.livingArrangements === LIVING_OTHER && !f.livingArrangementsOther) missing.add("livingArrangementsOther");
        if (f.employment === EMPLOYMENT_OTHER && !f.employmentOther) missing.add("employmentOther");
      }
      if (s === 2) {
        if (!drugRows.some((r) => r.drug)) missing.add("drug");
        // Each entered drug must be fully detailed (mirrors the legacy form's per-row `*`).
        drugRows.forEach((r, i) => {
          if (!r.drug) return; // empty placeholder row — ignore
          if (!r.ageOfFirstUse) missing.add(`row${i}.age`);
          if (!r.reason) missing.add(`row${i}.reason`);
          if (!r.usedLast3Months) missing.add(`row${i}.use3m`);
          if (!r.dailyUse) missing.add(`row${i}.daily`);
          if (!r.durationMonths) missing.add(`row${i}.duration`);
        });
        // Legacy form: only "Intravenous Drug Use Ever" is required; the needle-sharing
        // questions are conditional ("If yes…") and optional.
        if (!f.ivEver) missing.add("ivEver");
        // Sexual Practices + both test-history questions are required on the legacy form.
        for (const key of ["sexualPractices", "hcvHistory", "hbvHistory"]) if (!f[key]) missing.add(key);
      }
      if (s === 3) {
        for (const key of STEP_REQUIRED[3] ?? []) if (!f[key]) missing.add(key);
        if (f.previousTreatment === "Yes" && !f.treatmentTaken) missing.add("treatmentTaken");
        if (f.sourceOfReferral === REFERRAL_OTHER && !f.sourceOfReferralOther) missing.add("sourceOfReferralOther");
        if (moneySource.length === 0) missing.add("moneySource");
      }
      return missing;
    },
    [f, drugRows, moneySource],
  );

  const remainingThisStep = React.useMemo(() => missingForStep(step).size, [missingForStep, step]);

  /** Human label for an error key, including per-drug-row keys like `row0.age`. */
  const labelForKey = (k: string): string => {
    const row = k.match(/^row(\d+)\.(\w+)$/);
    if (row) {
      const fieldLabels: Record<string, string> = {
        age: "Age of First Use",
        reason: "Reason for Initiation/Use",
        use3m: "Use in Last 3 Months",
        daily: "Daily/Near-daily Use",
        duration: "Duration of Regular Use",
      };
      const subfield = row[2] ?? "";
      return `Drug ${Number(row[1]) + 1} — ${fieldLabels[subfield] ?? subfield}`;
    }
    return FIELD_LABELS[k] ?? k;
  };

  const flagStep = (missing: Set<string>, targetStep: number) => {
    setErrors(missing);
    setStep(targetStep);
    const names = [...missing].map(labelForKey);
    setSubmitError(
      names.length
        ? `Please complete ${names.length} field${names.length > 1 ? "s" : ""}: ${names.join(", ")}.`
        : "Please complete the highlighted fields before continuing.",
    );
    requestAnimationFrame(() => errorSummaryRef.current?.focus());
  };

  const goNext = () => {
    const missing = missingForStep(step);
    if (missing.size > 0) {
      flagStep(missing, step);
      return;
    }
    setErrors(new Set());
    setSubmitError("");
    setStep((s) => Math.min(s + 1, STEPS.length - 1));
  };

  const goBack = () => {
    setErrors(new Set());
    setSubmitError("");
    setStep((s) => Math.max(s - 1, 0));
  };

  const clearDraft = () => {
    try {
      sessionStorage.removeItem(DRAFT_KEY);
    } catch {
      /* ignore */
    }
    setF(INITIAL_FIELDS);
    setDrugRows([{ ...emptyDrugRow, _key: "row-0" }]);
    setSameAddress(false);
    setPhoto(null);
    setMoneySource([]);
    setErrors(new Set());
    setSubmitError("");
    setDraftRestored(false);
    setStep(0);
    toast("Draft cleared.", "success");
  };

  const submit = () => {
    if (submittingRef.current) return;
    for (const s of [0, 1, 2, 3]) {
      const missing = missingForStep(s);
      if (missing.size > 0) {
        flagStep(missing, s);
        return;
      }
    }
    setErrors(new Set());
    setSubmitError("");
    submittingRef.current = true;

    const referralLabel =
      f.sourceOfReferral === REFERRAL_OTHER && f.sourceOfReferralOther
        ? f.sourceOfReferralOther
        : labelOf(REFERRED_BY, f.sourceOfReferral);
    const moneySourceLabel = moneySource
      .map((v) => labelOf(MONEY_SOURCE_OPTIONS, v))
      .join(", ");

    const clinicalPairs: Array<[string, string]> = [
      ["IV drug use (ever)", labelOf(YES_NO_NR, f.ivEver)],
      ["IV drug use (last 3 months)", labelOf(YES_NO_NR, f.iv3m)],
      ["Shared needles (ever)", labelOf(YES_NO_NR, f.sharingEver)],
      ["Shared needles (last 3 months)", labelOf(YES_NO_NR, f.sharing3m)],
      ["Sexual practices", labelOf(SEXUAL_PRACTICES, f.sexualPractices)],
      ["HCV test history", labelOf(YES_NO, f.hcvHistory)],
      ["HCV result", labelOf(TEST_RESULT, f.hcvResult)],
      ["HBV test history", labelOf(YES_NO, f.hbvHistory)],
      ["HBV result", labelOf(TEST_RESULT, f.hbvResult)],
      ["ASSIST score (alcohol)", labelOf(ASSIST_SCORE_ALCOHOL, f.assistAlcohol)],
      ["ASSIST score (other drugs)", labelOf(ASSIST_SCORE_OTHER, f.assistOther)],
      ["Previous treatment", labelOf(YES_NO, f.previousTreatment)],
      ["Treatment taken from", f.previousTreatment === "Yes" ? labelOf(TREATMENT_TAKEN, f.treatmentTaken) : ""],
      ["Source of referral", referralLabel],
      ["Ever hospitalised", labelOf(YES_NO, f.everHospitalized)],
      ["Avg. daily expenditure (₹)", f.avgExpenditure],
      ["Source of money", moneySourceLabel],
      ["Ever apprehended", labelOf(YES_NO_NR, f.everApprehended)],
      ["Apprehended — for selling (ever)", labelOf(YES_NO, f.sellingEver)],
      ["Apprehended — for selling (last month)", labelOf(YES_NO, f.sellingLast)],
      ["Apprehended — for possession (ever)", labelOf(YES_NO, f.possessionEver)],
      ["Apprehended — for possession (last month)", labelOf(YES_NO, f.possessionLast)],
      ["Apprehended — unruly behaviour (ever)", labelOf(YES_NO, f.unrulyEver)],
      ["Apprehended — unruly behaviour (last month)", labelOf(YES_NO, f.unrulyLast)],
      ["Apprehended — other crime (ever)", labelOf(YES_NO, f.otherCrimeEver)],
      ["Apprehended — other crime (last month)", labelOf(YES_NO, f.otherCrimeLast)],
      ["Other crime (specify)", f.otherCrimeSpecify],
      ["Family history of substance use", labelOf(YES_NO, f.familyHistory)],
      ["Days since last use", f.daysSinceLast],
      ["Motivation at admission", labelOf(MOTIVATION_STAGES, f.motivation)],
    ];
    const clinicalDetails = Object.fromEntries(clinicalPairs.filter(([, v]) => v && v.trim()));

    const patient: Omit<Patient, "id"> = {
      // Unique demo registration number generated at submit time (event handler).
      // eslint-disable-next-line react-hooks/purity
      registrationNumber: `DM${Date.now().toString().slice(-8)}${crypto.randomUUID().slice(0, 4)}`,
      registrationProgress: "Pending",
      treatmentCenter: session.centerName,
      name: f.name,
      gender: labelOf(GENDERS, f.gender),
      age: Number(f.age) || 0,
      dateOfAdmission: f.dateOfAdmission,
      currentAddress: f.currentAddress,
      permanentAddress: f.permanentAddress,
      state: labelOf(STATES, f.state),
      district: labelOf(districtOptions, f.district),
      placeOfResidence: labelOf(PLACE_OF_RESIDENCE, f.placeOfResidence),
      maritalStatus: labelOf(MARITAL_STATUS, f.maritalStatus),
      livingArrangements:
        f.livingArrangements === LIVING_OTHER && f.livingArrangementsOther
          ? f.livingArrangementsOther
          : labelOf(LIVING_ARRANGEMENTS, f.livingArrangements),
      education: labelOf(EDUCATION, f.education),
      occupation: labelOf(OCCUPATION, f.occupation),
      employment:
        f.employment === EMPLOYMENT_OTHER && f.employmentOther
          ? f.employmentOther
          : labelOf(EMPLOYMENT, f.employment),
      income: labelOf(INCOME, f.income),
      category: labelOf(CATEGORY, f.category),
      contactNumber: f.contactNumber,
      governmentId: labelOf(GOVERNMENT_ID, f.governmentId),
      governmentIdNumber: f.governmentIdNumber,
      photoUrl: photo?.url,
      drugUse: drugRows
        .filter((r) => r.drug)
        .map((r) => ({
          drug: labelOf(DRUGS, r.drug),
          ageOfFirstUse: r.ageOfFirstUse,
          reason: labelOf(INITIATION_REASONS, r.reason),
          usedLast3Months: r.usedLast3Months,
          dailyUse: r.dailyUse,
          durationMonths: r.durationMonths,
        })),
      provisionalDiagnosis: labelOf(PROVISIONAL_DIAGNOSIS, f.provisionalDiagnosis),
      clinicalDetails: Object.keys(clinicalDetails).length ? clinicalDetails : undefined,
    };

    store.addPatient(patient);
    try {
      sessionStorage.removeItem(DRAFT_KEY);
    } catch {
      /* ignore */
    }
    toast("Patient registered successfully. Continue with the clinical record from the patient list.", "success");
    router.push("/portals/nmba/treatment-centre/irca/patients");
  };

  const err = (key: string): string | undefined => {
    if (!errors.has(key)) return undefined;
    if (key === "contactNumber" && f.contactNumber) return "Enter a valid 10-digit mobile number.";
    if (key === "age" && f.age) return "Enter a valid age between 1 and 120.";
    if (key === "dateOfAdmission" && f.dateOfAdmission) return "Date of admission cannot be in the future.";
    return "This field is required.";
  };

  const recordedDrugs = drugRows.filter((r) => r.drug);

  /** Reasons the patient was ever apprehended (for the review summary). */
  const apprehensions = LEGAL_ROWS.filter((row) => f[`${row.key}Ever`] === "Yes").map((row) => row.label);

  /** One-line detail string for a recorded drug (used in the review step). */
  const drugDetail = (r: DrugUseRow): string =>
    [
      r.ageOfFirstUse ? `First use age ${r.ageOfFirstUse}` : "",
      r.reason ? labelOf(INITIATION_REASONS, r.reason) : "",
      r.usedLast3Months ? `Used in last 3 months: ${r.usedLast3Months}` : "",
      r.dailyUse ? `Daily/near-daily: ${r.dailyUse}` : "",
      r.durationMonths ? `Regular use ${r.durationMonths} month${r.durationMonths === "1" ? "" : "s"}` : "",
    ]
      .filter(Boolean)
      .join("  ·  ");

  const jumpTo = (to: number) => {
    setErrors(new Set());
    setSubmitError("");
    setStep(to);
  };

  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        if (step === STEPS.length - 1) submit();
        else goNext();
      }}
      className="flex flex-col gap-6"
      noValidate
    >
      <div className="flex flex-col gap-1">
        <h1 className="text-headline-1 text-ink">Patient Registration &amp; Details Submission</h1>
        <p className="text-body-2 text-ink-muted">
          Register a new IRCA patient. Fields marked <span aria-hidden="true">*</span>
          <span className="sr-only">with an asterisk</span> are required. After registering, complete the
          clinical record (history, dosage, counselling, discharge) from the patient list.
        </p>
      </div>

      {draftRestored && (
        <Alert status="info">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <span>We restored your unsaved draft from this session. The photo needs to be re-attached.</span>
            <button type="button" onClick={clearDraft} className="font-semibold text-navy hover:underline">
              Start fresh
            </button>
          </div>
        </Alert>
      )}

      {step < STEPS.length - 1 && remainingThisStep > 0 && !submitError && (
        <p className="text-body-3 text-ink-muted">
          <span className="font-semibold text-ink">{remainingThisStep}</span> required field
          {remainingThisStep > 1 ? "s" : ""} remaining on this step.
        </p>
      )}

      <Wizard
        steps={STEPS}
        current={step}
        onBack={goBack}
        onNext={goNext}
        onSubmit={submit}
        submitLabel="Submit Registration"
        error={submitError || undefined}
        errorRef={errorSummaryRef}
      >
        {/* ---- Step 1: Patient Identity ---- */}
        {step === 0 && (
          <div className="flex flex-col gap-6">
            <FormCard title="Patient Photo" description="Optional — a recent passport-style photo of the patient.">
              <MediaUpload
                value={photo?.url}
                fileName={photo?.name}
                maxSizeMb={2}
                onChange={(url, name) => setPhoto({ url, name })}
                onClear={() => setPhoto(null)}
              />
            </FormCard>

            <FormSection title="Details of the Patient" columns={2}>
              <FormField label="Date of Admission" required error={err("dateOfAdmission")}>
                {(c) => (
                  <Input {...c} type="date" max={todayIso()} value={f.dateOfAdmission} onChange={(e) => set("dateOfAdmission")(e.target.value)} invalid={errors.has("dateOfAdmission")} />
                )}
              </FormField>
              <FormField label="Name of the Patient" required error={err("name")}>
                {(c) => (
                  <Input {...c} autoComplete="name" value={f.name} onChange={(e) => set("name")(e.target.value.replace(/[^a-zA-Z\s.]/g, ""))} placeholder="Full name" invalid={errors.has("name")} />
                )}
              </FormField>
              <FormField label="Gender" required error={err("gender")}>
                {(c) => (
                  <Select {...c} value={f.gender} onChange={(e) => set("gender")(e.target.value)} placeholder="Select Gender" options={GENDERS} invalid={errors.has("gender")} />
                )}
              </FormField>
              <FormField label="Age" required error={err("age")}>
                {(c) => (
                  <Input {...c} autoComplete="off" type="number" min={1} max={120} step={1} value={f.age} onChange={(e) => set("age")(e.target.value)} placeholder="Age in years" invalid={errors.has("age")} />
                )}
              </FormField>
              <FormField label="Contact Number" required error={err("contactNumber")}>
                {(c) => (
                  <Input {...c} autoComplete="tel-national" type="tel" inputMode="numeric" maxLength={10} value={f.contactNumber} onChange={(e) => set("contactNumber")(e.target.value.replace(/\D/g, ""))} placeholder="10-digit number" invalid={errors.has("contactNumber")} />
                )}
              </FormField>
              <FormField label="State" required error={err("state")}>
                {(c) => (
                  <Select {...c} value={f.state} onChange={(e) => { set("state")(e.target.value); set("district")(""); }} placeholder="Select State" options={STATES} invalid={errors.has("state")} />
                )}
              </FormField>
              <FormField label="District" required error={err("district")}>
                {(c) => (
                  <Select {...c} value={f.district} onChange={(e) => set("district")(e.target.value)} placeholder={f.state ? "Select District" : "Select a State first"} options={districtOptions} disabled={!f.state} invalid={errors.has("district")} />
                )}
              </FormField>
              <FormField label="Place of Residence" required error={err("placeOfResidence")}>
                {(c) => (
                  <Select {...c} value={f.placeOfResidence} onChange={(e) => set("placeOfResidence")(e.target.value)} placeholder="Select Residence" options={PLACE_OF_RESIDENCE} invalid={errors.has("placeOfResidence")} />
                )}
              </FormField>
            </FormSection>

            <FormSection title="Address" columns={2}>
              <FormField label="Current Address" required error={err("currentAddress")}>
                {(c) => (
                  <Textarea {...c} rows={3} value={f.currentAddress} onChange={(e) => { const v = e.target.value; set("currentAddress")(v); if (sameAddress) set("permanentAddress")(v); }} placeholder="House no., street, area" invalid={errors.has("currentAddress")} />
                )}
              </FormField>
              <FormField label="Permanent Address" required error={err("permanentAddress")}>
                {(c) => (
                  <Textarea {...c} rows={3} value={f.permanentAddress} onChange={(e) => set("permanentAddress")(e.target.value)} disabled={sameAddress} placeholder="House no., street, area" invalid={errors.has("permanentAddress")} />
                )}
              </FormField>
              <div className="flex items-center pt-0.5">
                <Checkbox checked={sameAddress} onChange={(e) => onSameAddress(e.target.checked)} label="Permanent address same as current" />
              </div>
            </FormSection>
          </div>
        )}

        {/* ---- Step 2: Socio-economic & ID ---- */}
        {step === 1 && (
          <FormSection title="Socio-economic Background & Identity" columns={2}>
            <FormField label="Marital Status" required error={err("maritalStatus")}>
              {(c) => <Select {...c} value={f.maritalStatus} onChange={(e) => set("maritalStatus")(e.target.value)} placeholder="Select Marital Status" options={MARITAL_STATUS} invalid={errors.has("maritalStatus")} />}
            </FormField>
            <FormField label="Living Arrangements" required error={err("livingArrangements")}>
              {(c) => <Select {...c} value={f.livingArrangements} onChange={(e) => set("livingArrangements")(e.target.value)} placeholder="Select Living Arrangements" options={LIVING_ARRANGEMENTS} invalid={errors.has("livingArrangements")} />}
            </FormField>
            {f.livingArrangements === LIVING_OTHER && (
              <FormField label="Living Arrangements (specify)" required error={err("livingArrangementsOther")}>
                {(c) => <Input {...c} value={f.livingArrangementsOther} onChange={(e) => set("livingArrangementsOther")(e.target.value)} placeholder="Specify living arrangement" invalid={errors.has("livingArrangementsOther")} />}
              </FormField>
            )}
            <FormField label="Educational Status" required error={err("education")}>
              {(c) => <Select {...c} value={f.education} onChange={(e) => set("education")(e.target.value)} placeholder="Select Education" options={EDUCATION} invalid={errors.has("education")} />}
            </FormField>
            <FormField label="Occupational Status" required error={err("occupation")}>
              {(c) => <Select {...c} value={f.occupation} onChange={(e) => set("occupation")(e.target.value)} placeholder="Select Occupation" options={OCCUPATION} invalid={errors.has("occupation")} />}
            </FormField>
            <FormField label="Employment Status" required error={err("employment")}>
              {(c) => <Select {...c} value={f.employment} onChange={(e) => set("employment")(e.target.value)} placeholder="Select Employment" options={EMPLOYMENT} invalid={errors.has("employment")} />}
            </FormField>
            {f.employment === EMPLOYMENT_OTHER && (
              <FormField label="Employment Status (specify)" required error={err("employmentOther")}>
                {(c) => <Input {...c} value={f.employmentOther} onChange={(e) => set("employmentOther")(e.target.value)} placeholder="Specify employment status" invalid={errors.has("employmentOther")} />}
              </FormField>
            )}
            <FormField label="Income (monthly)" required error={err("income")}>
              {(c) => <Select {...c} value={f.income} onChange={(e) => set("income")(e.target.value)} placeholder="Select Income" options={INCOME} invalid={errors.has("income")} />}
            </FormField>
            <FormField label="Category" required error={err("category")}>
              {(c) => <Select {...c} value={f.category} onChange={(e) => set("category")(e.target.value)} placeholder="Select Category" options={CATEGORY} invalid={errors.has("category")} />}
            </FormField>
            <FormField label="Government ID" required error={err("governmentId")}>
              {(c) => <Select {...c} value={f.governmentId} onChange={(e) => set("governmentId")(e.target.value)} placeholder="Select Government ID" options={GOVERNMENT_ID} invalid={errors.has("governmentId")} />}
            </FormField>
            <FormField label="Government ID Number" required error={err("governmentIdNumber")}>
              {(c) => <Input {...c} value={f.governmentIdNumber} onChange={(e) => set("governmentIdNumber")(e.target.value)} placeholder="Enter ID number" invalid={errors.has("governmentIdNumber")} />}
            </FormField>
          </FormSection>
        )}

        {/* ---- Step 3: Substance Use ---- */}
        {step === 2 && (
          <div className="flex flex-col gap-6">
            <FormCard
              title="Drug Use Details"
              required
              headingId={drugSectionId}
              description="Each substance gets its own card — add as many as needed."
            >
              {errors.has("drug") && (
                <div className="mb-4">
                  <Alert id="drug-error" status="error">Add at least one drug — select a substance below.</Alert>
                </div>
              )}
              <ol className="flex flex-col gap-4">
                {drugRows.map((row, i) => (
                  <li key={row._key} className="rounded-xl border border-line bg-surface-muted/50 p-4 sm:p-5">
                    {/* Card header: drug number + remove */}
                    <div className="mb-4 flex items-center justify-between gap-2">
                      <span className="inline-flex items-center gap-2 text-title-3 text-navy">
                        <span className="flex h-6 w-6 items-center justify-center rounded-full bg-navy/10 text-label-2 font-bold text-navy" aria-hidden="true">
                          {i + 1}
                        </span>
                        Drug {i + 1}
                      </span>
                      {drugRows.length > 1 && (
                        <button
                          type="button"
                          onClick={() => setDrugRows((prev) => prev.filter((_, idx) => idx !== i))}
                          aria-label={`Remove drug ${i + 1}`}
                          className="inline-flex h-9 items-center gap-1.5 rounded-lg px-2.5 text-label-2 font-semibold text-danger-fg hover:bg-danger-fg/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-danger-fg"
                        >
                          <Icon name="delete" size={14} aria-hidden /> Remove
                        </button>
                      )}
                    </div>

                    {/* Responsive field grid: 1 → 2 → 3 columns */}
                    <div className="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2 lg:grid-cols-3">
                      <FormField label="Drug" required error={errors.has("drug") && !row.drug ? "Select a drug." : undefined}>
                        {(c) => (
                          <Select {...c} value={row.drug} onChange={(e) => updateDrugRow(i, { drug: e.target.value })} placeholder="Select Drug" options={DRUGS} />
                        )}
                      </FormField>
                      <FormField label="Age of First Use" required error={errors.has(`row${i}.age`) ? "Required." : undefined}>
                        {(c) => (
                          <Input {...c} type="number" min={0} value={row.ageOfFirstUse} onChange={(e) => updateDrugRow(i, { ageOfFirstUse: e.target.value })} placeholder="Years" />
                        )}
                      </FormField>
                      <FormField label="Reason for Initiation / Use" required error={errors.has(`row${i}.reason`) ? "Required." : undefined}>
                        {(c) => (
                          <Select {...c} value={row.reason} onChange={(e) => updateDrugRow(i, { reason: e.target.value })} placeholder="Select Reason" options={INITIATION_REASONS} />
                        )}
                      </FormField>

                      <RadioGroup
                        legend="Use in Last 3 Months"
                        name={`used3m-${row._key}`}
                        required
                        orientation="horizontal"
                        options={YES_NO}
                        value={row.usedLast3Months || undefined}
                        onChange={(v) => updateDrugRow(i, { usedLast3Months: v as "Yes" | "No" })}
                        error={errors.has(`row${i}.use3m`) ? "Select Yes or No." : undefined}
                      />

                      <RadioGroup
                        legend="Daily / Near-daily Use"
                        name={`daily-${row._key}`}
                        required
                        orientation="horizontal"
                        options={YES_NO}
                        value={row.dailyUse || undefined}
                        onChange={(v) => updateDrugRow(i, { dailyUse: v as "Yes" | "No" })}
                        error={errors.has(`row${i}.daily`) ? "Select Yes or No." : undefined}
                      />

                      <FormField label="Duration of Regular Use (months)" required error={errors.has(`row${i}.duration`) ? "Required." : undefined}>
                        {(c) => (
                          <Input {...c} type="number" min={0} value={row.durationMonths} onChange={(e) => updateDrugRow(i, { durationMonths: e.target.value })} placeholder="Months" />
                        )}
                      </FormField>
                    </div>
                  </li>
                ))}
              </ol>
              <div className="mt-4">
                <Button type="button" appearance="outlined" iconLeft={<Icon name="add" size={16} />} onClick={() => setDrugRows((prev) => [...prev, { ...emptyDrugRow, _key: `row-${(keyRef.current += 1)}` }])}>
                  Add another drug
                </Button>
              </div>
            </FormCard>

            <FormSection title="Injecting Behaviour" columns={2}>
              <FormField label="Intravenous Drug Use Ever" required error={err("ivEver")}>
                {(c) => (
                  <Select
                    {...c}
                    value={f.ivEver}
                    onChange={(e) => {
                      set("ivEver")(e.target.value);
                      if (e.target.value !== "Yes") {
                        set("sharingEver")("");
                        set("sharing3m")("");
                      }
                    }}
                    placeholder="Select Option"
                    options={YES_NO_NR}
                    invalid={errors.has("ivEver")}
                  />
                )}
              </FormField>
              <FormField label="Intravenous Drug Use in Last 3 Months">
                {(c) => <Select {...c} value={f.iv3m} onChange={(e) => set("iv3m")(e.target.value)} placeholder="Select Option" options={YES_NO_NR} />}
              </FormField>
              {/* Needle/syringe sharing only applies if the patient injects ("If yes…"). */}
              {f.ivEver === "Yes" && (
                <>
                  <FormField label="Sharing Needles/Syringes Ever">
                    {(c) => <Select {...c} value={f.sharingEver} onChange={(e) => set("sharingEver")(e.target.value)} placeholder="Select Option" options={YES_NO_NR} />}
                  </FormField>
                  <FormField label="Sharing Needles/Syringes in Last 3 Months">
                    {(c) => <Select {...c} value={f.sharing3m} onChange={(e) => set("sharing3m")(e.target.value)} placeholder="Select Option" options={YES_NO_NR} />}
                  </FormField>
                </>
              )}
            </FormSection>

            <FormSection title="Sexual Behaviour" columns={2}>
              <FormField label="Sexual Practices" required error={err("sexualPractices")}>
                {(c) => <Select {...c} value={f.sexualPractices} onChange={(e) => set("sexualPractices")(e.target.value)} placeholder="Select Sexual Practices" options={SEXUAL_PRACTICES} invalid={errors.has("sexualPractices")} />}
              </FormField>
              <div className="hidden xl:block" />
              <FormField label="Ever had HCV testing — Test History" required error={err("hcvHistory")}>
                {(c) => <Select {...c} value={f.hcvHistory} onChange={(e) => set("hcvHistory")(e.target.value)} placeholder="Select Option" options={YES_NO} invalid={errors.has("hcvHistory")} />}
              </FormField>
              <FormField label="Result of HCV Test">
                {(c) => <Select {...c} value={f.hcvResult} onChange={(e) => set("hcvResult")(e.target.value)} placeholder="Select Option" options={TEST_RESULT} />}
              </FormField>
              <FormField label="Ever had HBV testing — Test History" required error={err("hbvHistory")}>
                {(c) => <Select {...c} value={f.hbvHistory} onChange={(e) => set("hbvHistory")(e.target.value)} placeholder="Select Option" options={YES_NO} invalid={errors.has("hbvHistory")} />}
              </FormField>
              <FormField label="Result of HBV Test">
                {(c) => <Select {...c} value={f.hbvResult} onChange={(e) => set("hbvResult")(e.target.value)} placeholder="Select Option" options={TEST_RESULT} />}
              </FormField>
            </FormSection>
          </div>
        )}

        {/* ---- Step 4: Assessment & Diagnosis ---- */}
        {step === 3 && (
          <div className="flex flex-col gap-6">
            <FormSection title="Alcohol, Smoking and Substance Involvement Screening Test (ASSIST) Score" columns={2}>
              <FormField label="ASSIST Score For Alcohol Use" required error={err("assistAlcohol")}>
                {(c) => <Select {...c} value={f.assistAlcohol} onChange={(e) => set("assistAlcohol")(e.target.value)} placeholder="Select ASSIST Score" options={ASSIST_SCORE_ALCOHOL} invalid={errors.has("assistAlcohol")} />}
              </FormField>
              <FormField label="ASSIST Score For Other Drugs" required error={err("assistOther")}>
                {(c) => <Select {...c} value={f.assistOther} onChange={(e) => set("assistOther")(e.target.value)} placeholder="Select ASSIST Score" options={ASSIST_SCORE_OTHER} invalid={errors.has("assistOther")} />}
              </FormField>
            </FormSection>

            <FormSection title="Treatment Details" columns={2}>
              <FormField label="Previous Treatment for Substance use" required error={err("previousTreatment")}>
                {(c) => <Select {...c} value={f.previousTreatment} onChange={(e) => { set("previousTreatment")(e.target.value); if (e.target.value !== "Yes") set("treatmentTaken")(""); }} placeholder="Select Previous Treatment" options={YES_NO} invalid={errors.has("previousTreatment")} />}
              </FormField>
              {f.previousTreatment === "Yes" && (
                <FormField label="If yes, treatment taken from" required error={err("treatmentTaken")}>
                  {(c) => <Select {...c} value={f.treatmentTaken} onChange={(e) => set("treatmentTaken")(e.target.value)} placeholder="Select Treatment Taken" options={TREATMENT_TAKEN} invalid={errors.has("treatmentTaken")} />}
                </FormField>
              )}
              <FormField label="Source of Referral" required error={err("sourceOfReferral")}>
                {(c) => <Select {...c} value={f.sourceOfReferral} onChange={(e) => { set("sourceOfReferral")(e.target.value); if (e.target.value !== REFERRAL_OTHER) set("sourceOfReferralOther")(""); }} placeholder="Select Source of Referral" options={REFERRED_BY} invalid={errors.has("sourceOfReferral")} />}
              </FormField>
              {f.sourceOfReferral === REFERRAL_OTHER && (
                <FormField label="Source of Referral (specify)" required error={err("sourceOfReferralOther")}>
                  {(c) => <Input {...c} value={f.sourceOfReferralOther} onChange={(e) => set("sourceOfReferralOther")(e.target.value)} placeholder="Specify referral source" invalid={errors.has("sourceOfReferralOther")} />}
                </FormField>
              )}
              <FormField label="Ever hospitalized for treatment of substance use" required error={err("everHospitalized")}>
                {(c) => <Select {...c} value={f.everHospitalized} onChange={(e) => set("everHospitalized")(e.target.value)} placeholder="Select Option" options={YES_NO} invalid={errors.has("everHospitalized")} />}
              </FormField>
            </FormSection>

            <FormCard title="Miscellaneous">
              <div className="ds-form-section__grid ds-form-section__grid--3">
                <FormField label="Average Daily Expenditure on Drugs" required error={err("avgExpenditure")}>
                  {(c) => <Input {...c} type="number" min={0} value={f.avgExpenditure} onChange={(e) => set("avgExpenditure")(e.target.value)} placeholder="₹ per day" invalid={errors.has("avgExpenditure")} />}
                </FormField>
                <div className="flex flex-col gap-1.5">
                  <span id="money-source-label" className="text-label-1 text-ink">
                    Source of Money for Drug Use <span className="ds-field__required">*</span>
                    <span className="ml-1 font-normal text-ink-muted">(select all that apply)</span>
                  </span>
                  <div
                    role="group"
                    aria-labelledby="money-source-label"
                    className={`flex flex-wrap gap-x-4 gap-y-2 rounded-lg border p-3 ${errors.has("moneySource") ? "border-danger-fg" : "border-line"}`}
                  >
                    {MONEY_SOURCE_OPTIONS.map((o) => (
                      <Checkbox
                        key={o.value}
                        checked={moneySource.includes(o.value)}
                        onChange={() => {
                          toggleMoneySource(o.value);
                          setErrors((prev) => {
                            const n = new Set(prev);
                            n.delete("moneySource");
                            return n;
                          });
                        }}
                        label={o.label}
                      />
                    ))}
                  </div>
                  {errors.has("moneySource") && (
                    <p className="text-label-2 text-danger-fg">Select at least one source.</p>
                  )}
                </div>
                <FormField label="Ever apprehended by police for drug-related offense" required error={err("everApprehended")}>
                  {(c) => <Select {...c} value={f.everApprehended} onChange={(e) => set("everApprehended")(e.target.value)} placeholder="Select Option" options={YES_NO_NR} invalid={errors.has("everApprehended")} />}
                </FormField>
              </div>

              {/* Criminal / legal history — flows after "Ever apprehended", as on the legacy form. */}
              <div className="mt-6 border-t border-line pt-5">
                <h3 id="legal-history-heading" className="text-title-3 text-navy">Criminal / Legal History</h3>
                <p className="mb-3 mt-1 text-body-3 text-ink-muted">If apprehended, record the reason — both ever and in the last month. Optional.</p>
                <div className="overflow-x-auto">
                  <table className="min-w-full text-body-2" aria-labelledby="legal-history-heading">
                    <thead>
                      <tr className="text-left text-label-3 uppercase text-ink-muted">
                        <th scope="col" className="px-2 py-2">Reason</th>
                        <th scope="col" className="px-2 py-2">Ever</th>
                        <th scope="col" className="px-2 py-2">Last One Month</th>
                      </tr>
                    </thead>
                    <tbody>
                      {LEGAL_ROWS.map((rowDef) => {
                        const everKey = `${rowDef.key}Ever`;
                        const lastKey = `${rowDef.key}Last`;
                        return (
                          <tr key={rowDef.key} className="align-top">
                            <th scope="row" className="px-2 py-2 text-left font-medium text-ink">{rowDef.label}</th>
                            <td className="px-2 py-2">
                              <RadioGroup legend={`${rowDef.label} — ever`} hideLegend name={everKey} orientation="horizontal" options={YES_NO} value={f[everKey] || undefined} onChange={(v) => set(everKey)(v)} />
                            </td>
                            <td className="px-2 py-2">
                              <RadioGroup legend={`${rowDef.label} — last one month`} hideLegend name={lastKey} orientation="horizontal" options={YES_NO} value={f[lastKey] || undefined} onChange={(v) => set(lastKey)(v)} />
                            </td>
                          </tr>
                        );
                      })}
                    </tbody>
                  </table>
                </div>
                <div className="mt-4 max-w-md">
                  <FormField label="Please specify (in case of any other crime)">
                    {(c) => <Input {...c} value={f.otherCrimeSpecify} onChange={(e) => set("otherCrimeSpecify")(e.target.value)} placeholder="Describe the other crime" />}
                  </FormField>
                </div>
              </div>

              <div className="ds-form-section__grid ds-form-section__grid--3 mt-6 border-t border-line pt-5">
                <FormField label="Any history of substance use in the family" required error={err("familyHistory")}>
                  {(c) => <Select {...c} value={f.familyHistory} onChange={(e) => set("familyHistory")(e.target.value)} placeholder="Select Option" options={YES_NO} invalid={errors.has("familyHistory")} />}
                </FormField>
                <FormField label="How many days ago was the substance last consumed?" required error={err("daysSinceLast")}>
                  {(c) => <Input {...c} type="number" min={0} value={f.daysSinceLast} onChange={(e) => set("daysSinceLast")(e.target.value)} placeholder="Days" invalid={errors.has("daysSinceLast")} />}
                </FormField>
                <FormField label="Patient's Motivation During the time of admission" required error={err("motivation")}>
                  {(c) => <Select {...c} value={f.motivation} onChange={(e) => set("motivation")(e.target.value)} placeholder="Select Patient's Motivation" options={MOTIVATION_STAGES} invalid={errors.has("motivation")} />}
                </FormField>
              </div>
            </FormCard>

            <FormSection title="Diagnosis" columns={2}>
              <FormField label="Provisional Diagnosis (as per ICD 11)" required error={err("provisionalDiagnosis")}>
                {(c) => <Select {...c} value={f.provisionalDiagnosis} onChange={(e) => set("provisionalDiagnosis")(e.target.value)} placeholder="Select Provisional Diagnosis" options={PROVISIONAL_DIAGNOSIS} invalid={errors.has("provisionalDiagnosis")} />}
              </FormField>
            </FormSection>
          </div>
        )}

        {/* ---- Step 5: Review ---- */}
        {step === 4 && (
          <div className="flex flex-col gap-5">
            <div className="flex flex-col gap-3">
              <p className="text-body-2 text-ink-muted">
                Review the details below, then submit. Use the quick links or{" "}
                <span className="font-semibold text-ink">Back</span> to make changes.
              </p>
              <div className="flex flex-wrap gap-2">
                {[
                  { to: 0, label: "Edit Identity" },
                  { to: 1, label: "Edit Socio-economic" },
                  { to: 2, label: "Edit Substance Use" },
                  { to: 3, label: "Edit Assessment" },
                ].map((j) => (
                  <button
                    key={j.to}
                    type="button"
                    onClick={() => jumpTo(j.to)}
                    className="inline-flex items-center gap-1 rounded-lg border border-line bg-white px-3 py-1.5 text-label-2 font-semibold text-navy hover:bg-black/5"
                  >
                    <Icon name="edit" size={12} aria-hidden /> {j.label}
                  </button>
                ))}
              </div>
            </div>

            <ReviewSection title="Patient Identity">
              <ReviewItem label="Photo" value={photo ? photo.name : "Not attached"} />
              <ReviewItem label="Date of Admission" value={f.dateOfAdmission} />
              <ReviewItem label="Name" value={f.name} />
              <ReviewItem label="Gender" value={labelOf(GENDERS, f.gender)} />
              <ReviewItem label="Age" value={f.age} />
              <ReviewItem label="Contact Number" value={f.contactNumber} />
              <ReviewItem label="Current Address" value={f.currentAddress} />
              <ReviewItem label="Permanent Address" value={f.permanentAddress} />
              <ReviewItem label="State" value={labelOf(STATES, f.state)} />
              <ReviewItem label="District" value={labelOf(districtOptions, f.district)} />
              <ReviewItem label="Place of Residence" value={labelOf(PLACE_OF_RESIDENCE, f.placeOfResidence)} />
            </ReviewSection>

            <ReviewSection title="Socio-economic & ID">
              <ReviewItem label="Marital Status" value={labelOf(MARITAL_STATUS, f.maritalStatus)} />
              <ReviewItem label="Living Arrangements" value={labelOf(LIVING_ARRANGEMENTS, f.livingArrangements)} />
              <ReviewItem label="Education" value={labelOf(EDUCATION, f.education)} />
              <ReviewItem label="Occupation" value={labelOf(OCCUPATION, f.occupation)} />
              <ReviewItem label="Employment" value={labelOf(EMPLOYMENT, f.employment)} />
              <ReviewItem label="Income" value={labelOf(INCOME, f.income)} />
              <ReviewItem label="Category" value={labelOf(CATEGORY, f.category)} />
              <ReviewItem label="Government ID" value={f.governmentId ? `${labelOf(GOVERNMENT_ID, f.governmentId)}: ${f.governmentIdNumber || ""}` : ""} />
            </ReviewSection>

            <ReviewSection title="Substance Use">
              {/* Drugs — each recorded substance as a full-width row with its details. */}
              {recordedDrugs.length === 0 ? (
                <ReviewItem wide label="Drugs recorded" value="None" />
              ) : (
                recordedDrugs.map((r, i) => (
                  <ReviewItem
                    key={r._key}
                    wide
                    label={`Drug ${i + 1} — ${labelOf(DRUGS, r.drug)}`}
                    value={drugDetail(r)}
                  />
                ))
              )}

              {/* Injecting behaviour. */}
              <ReviewItem label="IV drug use (ever)" value={labelOf(YES_NO_NR, f.ivEver)} />
              <ReviewItem label="IV drug use (last 3 months)" value={labelOf(YES_NO_NR, f.iv3m)} />
              {f.ivEver === "Yes" && (
                <>
                  <ReviewItem label="Shared needles (ever)" value={labelOf(YES_NO_NR, f.sharingEver)} />
                  <ReviewItem label="Shared needles (last 3 months)" value={labelOf(YES_NO_NR, f.sharing3m)} />
                </>
              )}

              {/* Sexual behaviour & blood-borne virus testing. */}
              <ReviewItem label="Sexual practices" value={labelOf(SEXUAL_PRACTICES, f.sexualPractices)} />
              <ReviewItem label="HCV testing — history" value={labelOf(YES_NO, f.hcvHistory)} />
              <ReviewItem label="HCV result" value={labelOf(TEST_RESULT, f.hcvResult)} />
              <ReviewItem label="HBV testing — history" value={labelOf(YES_NO, f.hbvHistory)} />
              <ReviewItem label="HBV result" value={labelOf(TEST_RESULT, f.hbvResult)} />
            </ReviewSection>

            <ReviewSection title="Assessment & Diagnosis">
              <ReviewItem label="ASSIST (alcohol)" value={labelOf(ASSIST_SCORE_ALCOHOL, f.assistAlcohol)} />
              <ReviewItem label="ASSIST (other drugs)" value={labelOf(ASSIST_SCORE_OTHER, f.assistOther)} />
              <ReviewItem label="Previous treatment" value={labelOf(YES_NO, f.previousTreatment)} />
              {f.previousTreatment === "Yes" && (
                <ReviewItem label="Treatment taken from" value={labelOf(TREATMENT_TAKEN, f.treatmentTaken)} />
              )}
              <ReviewItem
                label="Source of Referral"
                value={
                  f.sourceOfReferral === REFERRAL_OTHER && f.sourceOfReferralOther
                    ? f.sourceOfReferralOther
                    : labelOf(REFERRED_BY, f.sourceOfReferral)
                }
              />
              <ReviewItem label="Ever hospitalised" value={labelOf(YES_NO, f.everHospitalized)} />
              <ReviewItem label="Avg. daily expenditure" value={f.avgExpenditure ? `₹${f.avgExpenditure}` : ""} />
              <ReviewItem label="Source of money" value={moneySource.map((v) => labelOf(MONEY_SOURCE_OPTIONS, v)).join(", ")} />
              <ReviewItem label="Ever apprehended by police" value={labelOf(YES_NO_NR, f.everApprehended)} />
              <ReviewItem label="Family history of substance use" value={labelOf(YES_NO, f.familyHistory)} />
              <ReviewItem label="Days since last consumed" value={f.daysSinceLast} />
              <ReviewItem label="Motivation" value={labelOf(MOTIVATION_STAGES, f.motivation)} />
              <ReviewItem label="Provisional Diagnosis" value={labelOf(PROVISIONAL_DIAGNOSIS, f.provisionalDiagnosis)} />
              <ReviewItem
                wide
                label="Apprehensions (ever)"
                value={apprehensions.length ? apprehensions.join(", ") : "None recorded"}
              />
            </ReviewSection>
          </div>
        )}
      </Wizard>
    </form>
  );
}
