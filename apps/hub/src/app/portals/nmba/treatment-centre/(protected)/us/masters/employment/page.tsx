...
import { EMPLOYMENT as INITIAL_EMPLOYMENT } from "@/lib/nmba/treatment-centre/master-data";
...
export default function USEmploymentMasterPage() {
  const { toast } = useToast();
  const [items, setItems] = React.useState<MasterItem[]>(
    INITIAL_EMPLOYMENT.map((d, i) => ({ sno: i + 1, label: d.label, value: d.value }))
  );
  ...
  const handleDelete = (item: MasterItem) => {
    if (confirm(`Are you sure you want to delete "${item.label}"?`)) {
      ...
      toast("Employment status deleted successfully.", "warning");
    }
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editingItem) {
      ...
      toast("Employment status updated successfully.", "success");
    } else {
      if (items.some((i) => i.value === value)) {
        toast("Employment status code (value) must be unique.", "error");
        return;
      }
      setItems((prev) => [...prev, { sno: prev.length + 1, label, value }]);
      toast("Employment status added successfully.", "success");
    }
    setModalOpen(false);
  };
  ...
  const columns: ColumnDef<MasterItem>[] = [
    { key: "sno", header: "S.No" },
    { key: "label", header: "Employment Status" },
  ...
  return (
    <>
      <TCListPage
        title="Employment Status Master"
        columns={columns}
        data={items}
        searchKeys={["label", "value"]}
        fileName="employment-status-master"
        action={
          <button
            type="button"
          ...
          >
            <Plus className="h-4 w-4" aria-hidden /> Add New Employment Status
          </button>
        }
      />
      ...
      <Modal
        ...
        title={editingItem ? "Edit Employment Status details" : "Add New Employment Status"}
        footer={
          <>
            <Button type="button" appearance="outlined" onClick={() => setModalOpen(false)}>Cancel</Button>
            <Button type="submit" form="employment-form">Save</Button>
          </>
        }
      >
        <form id="employment-form" onSubmit={handleSave} className="flex flex-col gap-4">
          <FormField label="Employment Status" required>
            {(c) => <Input {...c} value={label} onChange={(e) => setLabel(e.target.value)} placeholder="e.g. Employed" />}
          </FormField>
