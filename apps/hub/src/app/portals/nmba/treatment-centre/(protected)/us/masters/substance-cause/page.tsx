...
import { INITIATION_REASONS as INITIAL_INITIATION_REASONS } from "@/lib/nmba/treatment-centre/master-data";
...
export default function USSubstanceCauseMasterPage() {
  const { toast } = useToast();
  const [items, setItems] = React.useState<MasterItem[]>(
    INITIAL_INITIATION_REASONS.map((d, i) => ({ sno: i + 1, label: d.label, value: d.value }))
  );
  ...
  const handleDelete = (item: MasterItem) => {
    if (confirm(`Are you sure you want to delete "${item.label}"?`)) {
      ...
      toast("Cause deleted successfully.", "warning");
    }
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editingItem) {
      ...
      toast("Cause updated successfully.", "success");
    } else {
      if (items.some((i) => i.value === value)) {
        toast("Cause code (value) must be unique.", "error");
        return;
      }
      setItems((prev) => [...prev, { sno: prev.length + 1, label, value }]);
      toast("Cause added successfully.", "success");
    }
    setModalOpen(false);
  };
  ...
  const columns: ColumnDef<MasterItem>[] = [
    { key: "sno", header: "S.No" },
    { key: "label", header: "Cause of Substance Use" },
    { key: "value", header: "Cause Code (Value)" },
    {
      key: "actions",
      header: "Action",
      render: (r) => (
  ...
  return (
    <>
      <TCListPage
        title="Cause of Substance Use Master"
        columns={columns}
        data={items}
        searchKeys={["label", "value"]}
        fileName="substance-cause-master"
        action={
          <button
            type="button"
          ...
          >
            <Plus className="h-4 w-4" aria-hidden /> Add New Cause
          </button>
        }
      />
      ...
      <Modal
        ...
        title={editingItem ? "Edit Cause details" : "Add New Cause"}
        footer={
          <>
            <Button type="button" appearance="outlined" onClick={() => setModalOpen(false)}>Cancel</Button>
            <Button type="submit" form="substance-cause-form">Save</Button>
          </>
        }
      >
        <form id="substance-cause-form" onSubmit={handleSave} className="flex flex-col gap-4">
          <FormField label="Cause of Substance Use" required>
            {(c) => <Input {...c} value={label} onChange={(e) => setLabel(e.target.value)} placeholder="e.g. Peer Pressure" />}
          </FormField>
          <FormField label="Cause Code (Value)" required>
            {(c) => (
              <Input
                {...c}
                value={value}
