  ...
  return (
    <div>
      ...
      <Card className="mt-6 p-6">
        <h2 className="text-title-2 text-ink">Category Breakdown</h2>
        <p className="text-body-3 text-ink-hint">Disbursed relief by grievance category</p>
        {byCat.length === 0 ? (
          <p className="mt-6 rounded-lg bg-surface-muted px-4 py-8 text-center text-body-2 text-ink-hint">No category data yet — process a disbursement to populate this report.</p>
        ) : (
          <div className="mt-6 space-y-4">
            {byCat.map(([cat, val]) => (
              <div key={cat}>
                <div className="mb-1 flex justify-between text-body-3"><span className="text-ink-muted line-clamp-1">{cat}</span><span className="font-semibold text-ink">{fmtINR(val)}</span></div>
