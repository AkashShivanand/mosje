  ...
  return (
    <div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card className="p-6">
          <h2 className="text-sm font-bold text-ink">Top States by Grievances</h2>
          <p className="text-xs text-ink-hint">Highest grievance volume this FY</p>
          <div className="mt-5 space-y-4">
            {topStates.map(([st, v]) => (
              <div key={st}>
                <div className="mb-1 flex justify-between text-xs"><span className="text-ink-muted">{st}</span><span className="font-semibold text-ink">{v}</span></div>
        ...
        <Card className="p-6">
          ...
          <p className="mt-4 text-center text-xs text-ink-hint">National average · above the 90% target</p>
        </Card>
      </div>
    </div>
  );
