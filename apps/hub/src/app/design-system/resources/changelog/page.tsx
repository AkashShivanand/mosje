...
const RELEASES: Release[] = [
  {
    ...
    changes: [
      ...
      { kind: "Changed", text: "THE MASTHEAD REVERSES ITS `fontSize={false}`. The old rationale — \u201cthe UX4G widget is the single mechanism; a second stepper doubles up\u201d — rested on a premise that was never true in practice: the stepper was INERT, so it was not a competing mechanism, it was a dead control. Now that it works, the bar is the direct, visible way to resize text and the widget's floating button is hidden wherever the bar offers the same entry. One door, not two. CONSEQUENCE, TRACKED AND NOT YET DONE: the library's 13 nested AccessibilityBar instances and `navbar.md`'s Anatomy still say OFF and must be flipped to match" },
