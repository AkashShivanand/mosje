...
const RELEASES: Release[] = [
  {
    ...
    current: true,
    changes: [
      { kind: "Fixed", text: "THE OVERFLOW ROW LOOKED BROKEN, AND A SCREENSHOT SHOWED IT WHERE MEASUREMENTS HAD NOT. A native scrollbar ran THROUGH the filled segmented control, \u201cOverview\u201d rendered as \u201crview\u201d and \u201cSkill development\u201d as \u201cSkill d\u201d, and the selected pill was sliced at its rounded corner. Three fixes: the native scrollbar is hidden — ONLY in overflow mode, where the menu is an alternative affordance, never when it is the sole one; tabs `scroll-snap` to the start edge so the row always rests on a whole tab; and measured per-direction edge fades (`mask-image`, not an overlay, because the component sits on two different surfaces and a mask is colour-agnostic) make the trailing cut read as \u201cmore this way\u201d instead of damage. The trigger\u2019s gap went from 2px to 8px so it stops reading as part of the track" },
