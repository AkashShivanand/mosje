...
const RELEASES: Release[] = [
  {
    ...
    changes: [
      ...
      { kind: "Changed", text: "SITEHEADER IS MIGRATED ONTO THE SHARED COMPONENT, and its own hand-rolled Tier-1 bar is deleted — the `.ds-hdr-util*` block is gone from header.css, leaving AccessibilityBar as the single implementation. NOTE THIS CHANGES PUBLIC-SITE BEHAVIOUR: SiteHeader now renders the bar with `fontSize` enabled, so the A−/A/A+ stepper is surfaced on the live masthead, where previously the UX4G widget was the only font-size mechanism. An earlier draft of this entry recorded the migration as deliberately NOT done; the branch went on to do it, and this entry is corrected to what shipped" },
