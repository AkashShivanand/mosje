    ...
    <section className="wn-pmq" aria-labelledby="wn-pmq-title">
      ...
      <div className="sa-container">
        ...
        <div className="wn-pmq__card">
          {/* THE PHOTOGRAPH LEADS THE DOCUMENT, and the reference puts it on the
              left of the desktop card and the top of the phone card — so one
              document order serves both and nothing has to be reordered in CSS
              (WCAG 1.3.2). Its panel is the saffron tint the reference names. */}
          ...
          <div className="wn-pmq__body">
            {/* The mark is the design's own vector, exported from the card's
                `Quote Mark` node. It is not an <Icon>: Material Symbols
                publishes only `format_quote`, which is a CLOSING mark, and its
                fill belongs in CSS so the mark follows the brand mode rather
                than carrying an exported hex. Decorative — the quotation is
            ...
            <div className="wn-pmq__foot">
              <div className="wn-pmq__credit">
                <p className="wn-pmq__who">{ATTRIBUTION}</p>
                <p className="wn-pmq__where">
                  {EVENT} &middot; <time dateTime={DATE_TIME}>{DATE}</time>
                </p>
              </div>
              <Button
                href={SOURCE_PASSAGE}
