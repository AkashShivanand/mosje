      ...
      <div className="flex rounded-lg bg-surface-muted p-1">
        {(["non", "rec"] as const).map((t) => (
          <button key={t} type="button" onClick={() => setTab(t)} aria-pressed={tab === t}
            className={`flex-1 rounded-md px-3 py-2 text-[14px] font-semibold transition-colors ${tab === t ? "bg-white text-gov-blue-dark shadow-sm" : "text-ink-muted hover:text-ink"}`}>
...
export function PledgeBanner() {
  return (
    <div className="overflow-hidden rounded-2xl bg-gov-blue-dark px-6 py-7 sm:px-8">
