...
export function Checkbox({
  ...
  return (
    <label className={cn("flex cursor-pointer items-start gap-2.5 text-label-1 text-ink", className)}>
      <input
        type="checkbox"
        {...props}
...
export function RadioRow({
  ...
  return (
    <div className="flex flex-wrap gap-x-6 gap-y-2">
      {options.map((o) => (
        <label key={o} className="flex cursor-pointer items-center gap-2 text-label-1 text-ink">
          <input
            type="radio"
            name={name}
            checked={value === o}
