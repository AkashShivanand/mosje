...
export default function IrcaAwarenessPage() {
  const store = useTCStore();
  const rows: Row[] = store.awareness.map((p, i) => ({ ...p, sno: i + 1 }));
  return <TCListPage title="Details of Awareness Generation Program" columns={columns} data={rows} fileName="irca-awareness" />;
}
