...
import { INCOME as INITIAL_INCOME } from "@/lib/treatment-centre/master-data";
...
export default function USIncomeMasterPage() {
  const { toast } = useToast();
  const [items, setItems] = React.useState<MasterItem[]>(
    INITIAL_INCOME.map((d, i) => ({ sno: i + 1, label: d.label, value: d.value }))
  );
  ...
  const handleDelete = (item: MasterItem) => {
    if (confirm(`Are you sure you want to delete "${item.label}"?`)) {
      ...
      toast("Income slab deleted successfully.", "warning");
    }
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editingItem) {
      ...
      toast("Income slab updated successfully.", "success");
    } else {
      if (items.some((i) => i.value === value)) {
        toast("Income slab code (value) must be unique.", "error");
        return;
      }
      setItems((prev) => [...prev, { sno: prev.length + 1, label, value }]);
      toast("Income slab added successfully.", "success");
    }
    setModalOpen(false);
  };
  ...
  const columns: ColumnDef<MasterItem>[] = [
    { key: "sno", header: "S.No" },
    { key: "label", header: "Income Slab" },
    { key: "value", header: "Slab Code (Value)" },
    {
      key: "actions",
      header: "Action",
      render: (r) => (
  ...
  return (
    <>
      <TCListPage
        title="Income Slab Master"
        columns={columns}
        data={items}
        searchKeys={["label", "value"]}
        fileName="income-slab-master"
        action={
          <button
            type="button"
      ...
      <Modal
        ...
        title={editingItem ? "Edit Income Slab details" : "Add New Income Slab"}
        footer={
          <>
            <Button type="button" appearance="outlined" onClick={() => setModalOpen(false)}>Cancel</Button>
            <Button type="submit" form="income-form">Save</Button>
          </>
        }
      >
        <form id="income-form" onSubmit={handleSave} className="flex flex-col gap-4">
          <FormField label="Income Slab" required>
            {(c) => <Input {...c} value={label} onChange={(e) => setLabel(e.target.value)} placeholder="e.g. Below ₹50,000" />}
          </FormField>
          <FormField label="Slab Code (Value)" required>
            {(c) => (
              <Input
                {...c}
                value={value}
