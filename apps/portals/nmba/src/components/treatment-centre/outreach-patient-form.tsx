...
export function OutreachPatientForm({ redirectTo }: { redirectTo: string }) {
  ...
  const labelForKey = (k: string): string => {
    ...
    if (row) {
      ...
      return `Substance ${Number(row[1]) + 1} — ${fieldLabels[row[2]] ?? row[2]}`;
    }
        ...
        {step === 1 && (
          <div className="flex flex-col gap-6">
            ...
            >
              ...
              <div className="mt-4">
                <Button type="button" appearance="outlined" iconLeft={<Plus className="h-4 w-4" />} onClick={() => setDrugRows((prev) => [...prev, { ...EMPTY_DRUG_ROW, _key: `row-${(keyRef.current += 1)}` }])}>
                  Add
                </Button>
              </div>
            </FormCard>
