...
import { PERMISSION_MATRIX, ROLES } from "@/lib/mock-data";
...
export default function RoleEditPage() {
  ...
  return (
    <div className="space-y-lg">
      <PageHeader
        breadcrumbs={[{ label: "Access Control" }, { label: "Roles", href: "/roles" }, { label: role.name }]}
        title={`Edit · ${role.name}`}
        subtitle="Adjust scope and permissions for this role. Changes propagate after audit approval."
        actions={
          <div className="flex items-center gap-sm">
            <Button variant="outline" size="sm" asChild><Link href="/roles"><ArrowLeft className="h-3.5 w-3.5" /> Cancel</Link></Button>
            <Button size="sm" onClick={() => router.push("/roles")}><Save className="h-3.5 w-3.5" /> Save changes</Button>
          </div>
        }
      />
