...
import { PageHeader, Select, SearchInput, StatusPill, SlaBadge, Button, Table } from "@/components/ui";
import { useTg } from "@/lib/store/store";
import { ROLES } from "@/lib/roles";
import { kpisForRole, queueForRole, byState, approvalRateByState } from "@/lib/selectors";
  ...
  const columns = [
    ...
    { key: "risk", header: "Risk", render: (a: Application) => <SlaBadge daysLeft={a.slaDaysLeft} /> },
    {
      ...
      render: (a: Application) => (
        <Link href={`/admin/applications/${a.id}`} className="font-semibold text-navy hover:underline">
          View
        </Link>
      ),
    },
