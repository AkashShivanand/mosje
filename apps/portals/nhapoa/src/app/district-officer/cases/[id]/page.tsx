...
import { Card, StatusPill, Button, Textarea, Field } from "@/components/ui";
import { SlaPill, PriorityBadge } from "@/components/case-views";
import { useNhapoa } from "@/lib/store/store";
import { doActionsFor, fmtDate, fmtINR } from "@/lib/case-helpers";
...
export default function CaseDetailPage() {
  ...
  if (!c) {
    return (
      <div>
        <Link href="/district-officer/cases" className="inline-flex items-center gap-1 text-sm font-semibold text-navy hover:underline"><ArrowLeft className="h-4 w-4" /> Back to My Cases</Link>
  ...
  return (
    <div>
      ...
      <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="font-mono text-xl font-bold text-navy">{c.refNo}</h1>
      ...
      <div className="mt-6 flex flex-wrap gap-1 border-b border-line">
        {TABS.map((t) => (
          <button
            ...
            className={`-mb-px border-b-2 px-4 py-2.5 text-sm font-semibold transition-colors ${tab === t.key ? "border-navy text-navy" : "border-transparent text-ink-muted hover:text-ink"}`}
          >
            {t.label}
          </button>
        ))}
      ...
      <div className="mt-6">
        ...
        {tab === "clarifications" && <Card className="p-6 text-sm text-ink-muted">No clarifications raised on this case.</Card>}
        {tab === "documents" && <Card className="p-6 text-sm text-ink-muted">Supporting documents submitted with the grievance appear here.</Card>}
        {tab === "audit" && (
          <Card className="p-6">
...
function DetailBlock({ title, rows }: { title: string; rows: [string, string | undefined][] }) {
  return (
    <Card className="p-6">
      <p className="mb-4 text-xs font-bold uppercase tracking-wide text-ink-hint">{title}</p>
