...
import { Button, Field, TextInput, Textarea, Select, Card } from "@/components/ui";
import { RESCUE_GENDERS } from "@/lib/citizen-data";
import { STATES, DISTRICTS } from "@/lib/store/seed";
...
export default function RegisterRescuePage() {
    ...
    return (
      <CitizenShell>
        <Card className="mx-auto max-w-xl p-10 text-center">
          ...
          <Link href="/" className="mt-6 inline-block rounded-lg bg-navy px-5 py-2.5 text-sm font-semibold text-white hover:bg-navy-800">Back to Home</Link>
        </Card>
      </CitizenShell>
    );
  }
  ...
  return (
    <CitizenShell>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-ink">Register a Rescue</h1>
        <p className="mt-1 text-sm text-ink-muted">Quick distress report. Only a few fields — name, mobile (we&apos;ll send an OTP), location, and the problem.</p>
      </div>

      <Card className="p-6 sm:p-8">
        <p className="mb-6 text-xs font-bold uppercase tracking-wide text-ink-hint">Rescue Details · all fields are mandatory</p>
