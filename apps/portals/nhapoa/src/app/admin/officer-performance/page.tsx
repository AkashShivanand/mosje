          ...
          <tbody className="divide-y divide-line">
            {officers.map((u) => (
              <tr key={u.id} className="hover:bg-surface-muted/60">
                ...
                <td className="px-5 py-4"><span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold ${u.active ? "bg-approve-bg text-approve-fg" : "bg-slate-100 text-slate-500"}`}>{u.active ? "Active" : "Inactive"}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
