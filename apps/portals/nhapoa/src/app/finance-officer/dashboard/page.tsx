...
import { fmtINR } from "@/lib/case-helpers";
    ...
    <div>
      ...
      <Card className="mt-6 p-6">
        <div className="flex items-center justify-between">
          ...
          <Link href="/finance-officer/queue" className="inline-flex items-center gap-1 text-xs font-semibold text-navy hover:underline">View all <ArrowRight className="h-3.5 w-3.5" /></Link>
        </div>
        <ul className="mt-4 divide-y divide-line">
          ...
          {queue.slice(0, 6).map((c) => (
            <li key={c.id} className="flex items-center justify-between gap-3 py-3">
              <Link href="/finance-officer/queue" className="min-w-0">
