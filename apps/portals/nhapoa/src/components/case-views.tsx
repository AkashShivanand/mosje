...
import { slaDaysLeft, slaTone, slaLabel, priorityOf, fmtDate } from "@/lib/case-helpers";
...
export function SlaPill({ case: c }: { case: Case }) {
  ...
  return <span className={cn("inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold", SLA_TONE_CLASS[slaTone(left)])}>{slaLabel(left)}</span>;
}
...
export function SimpleCaseTable({
  ...
  if (cases.length === 0) return <div className="rounded-2xl border border-dashed border-line bg-white/60 px-6 py-16 text-center text-sm text-ink-hint">{emptyLabel}</div>;
  return (
  ...
  if (cases.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-line bg-white/60 px-6 py-16 text-center">
        <div className="text-sm font-semibold text-ink">No cases here</div>
        <div className="mt-1 text-xs text-ink-hint">Cases will appear as they reach this stage.</div>
      </div>
    );
  }
  return (
          ...
          {cases.map((c) => (
            <tr key={c.id} className="hover:bg-surface-muted/60">
              <td className="px-5 py-4 align-top">
                <Link href={`${detailBase}/${c.id}`} className="font-mono text-xs font-semibold text-navy hover:underline">{c.refNo}</Link>
                <PriorityBadge case={c} />
              </td>
              <td className="px-5 py-4 align-top">
                <div className="font-medium text-ink">{c.complainant.name}</div>
                <div className="text-xs text-ink-hint">{c.complainantRole}</div>
              </td>
