...
import { Button, Field, Fieldset, TextInput, Textarea, Select, RadioRow, Checkbox, Stepper, Card } from "@/components/ui";
import { GRIEVANCE_TYPES, SUBMISSION_ROLES } from "@/lib/citizen-data";
import { STATES, DISTRICTS } from "@/lib/store/seed";
import { useNhapoa } from "@/lib/store/store";
import type { CaseType, CaseSource, ComplainantRole } from "@/lib/store/types";
...
export function GrievanceWizard({
  ...
  function submit() {
    const c = createGrievance({
      ...
      category: d.category,
      state: d.incState,
      district: d.incDistrict || districtsFor(d.incState)[0],
      source,
